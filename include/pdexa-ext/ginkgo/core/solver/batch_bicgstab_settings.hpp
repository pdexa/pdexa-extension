...
namespace gko::kernels::batch_bicgstab {
...
struct settings {
  static_assert(std::is_same<RealType, remove_complex<RealType>>::value, "Template parameter must be a real type");
  int max_iterations = 0;
  RealType residual_tol = 0.0;
  batch::stop::tolerance_type tol_type = batch::stop::tolerance_type::absolute;
};
...
int local_memory_requirement(const int num_rows, const int num_rhs) {
  return (9 * num_rows * num_rhs) * static_cast<int>(sizeof(ValueType));
}

struct storage_config {
  // preconditioner storage
  bool prec_shared = false;
  // total number of shared vectors
  int n_shared = 0;
  // number of vectors in global memory
  int n_global = 0;
  // global stride from one batch entry to the next
  int gmem_stride_bytes = 0;
  // padded vector length
  int padded_vec_len = 0;
};

template<int align_bytes>
void set_gmem_stride_bytes(storage_config& sconf, const int multi_vector_size_bytes, const int prec_storage_bytes) {
  int gmem_stride = sconf.n_global * multi_vector_size_bytes;
  if (!sconf.prec_shared) { gmem_stride += prec_storage_bytes; }
...
storage_config
compute_shared_storage(const int available_shared_mem, const int num_rows, const int num_nz, const int num_rhs) {
  const int vec_size = num_rows * num_rhs * sizeof(ValueType);
  const int num_main_vecs = 9;
  const int prec_storage = 0; // Prectype::dynamic_work_size(num_rows, num_nz);
  ...
  storage_config sconf{false, 0, num_main_vecs, 0, num_rows * num_rhs};
