...
struct create_view_fn {
  ...
  template<typename ValueType>
  constexpr matrix::dense::uniform_batch<ValueType> operator()(matrix::Dense<ValueType>* vec) const {
    return {vec->get_values(), vec->get_num_batch_items(), static_cast<int32>(vec->get_common_size()[1]),
            static_cast<int32>(vec->get_common_size()[0]), static_cast<int32>(vec->get_common_size()[1])};
  }

  template<typename ValueType>
  constexpr matrix::dense::uniform_batch<const ValueType> operator()(const matrix::Dense<ValueType>* vec) const {
    return {vec->get_values(), vec->get_num_batch_items(), static_cast<int32>(vec->get_common_size()[1]),
            static_cast<int32>(vec->get_common_size()[0]), static_cast<int32>(vec->get_common_size()[1])};
  }
