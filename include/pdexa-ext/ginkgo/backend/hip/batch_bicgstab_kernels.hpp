...
#if PDEXA_EXT_ENABLE_HIP && __HIPCC__
...
void apply(std::shared_ptr<const DefaultExecutor> exec,
  ...
  const size_t prec_size = Prec::dynamic_work_size(padded_num_rows, mat);
  const auto sconf = kernels::batch_bicgstab::compute_shared_storage<Prec, hip_value_type>(
