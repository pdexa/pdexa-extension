...
#include "pdexa-ext/ginkgo/backend/cuda_hip/base/runtime.hpp"
#include "pdexa-ext/ginkgo/backend/cuda_hip/base/types.hpp"
#include "pdexa-ext/ginkgo/backend/cuda_hip/components/cooperative_groups.hpp"
#include "pdexa-ext/ginkgo/backend/cuda_hip/components/thread_ids.hpp"
 ...
 * considered.
 */
template<typename Group, typename ValueType, typename = std::enable_if_t<group::is_communicator_group<Group>::value>>
__device__ __forceinline__ int choose_pivot(const Group& group, ValueType local_data, bool is_pivoted) {
  using real = remove_complex<ValueType>;
  real lmag = is_pivoted ? -one<real>() : abs(local_data);
  const auto pivot = reduce(group, group.thread_rank(), [&](int lidx, int ridx) {
...
__device__ void reduce(const Group& __restrict__ group, ValueType* __restrict__ data, Operator reduce_op = Operator{}) {
  ...
  if (warp.thread_rank() == 0) { data[0] = result; }
}
...
__device__ void reduce_array(size_type size,
  ...
  for (auto i = tidx; i < size; i += blockDim.x * gridDim.x) { thread_result = reduce_op(thread_result, source[i]); }
