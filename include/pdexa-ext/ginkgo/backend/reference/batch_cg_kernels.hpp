...
namespace reference::batch_template {
namespace batch_single_kernels::batch_cg {

template<typename BatchMatrixType_entry, typename ValueType>
void initialize(const BatchMatrixType_entry& A_entry,
...
void update_p(const batch::multi_vector::batch_item<const ValueType>& rho_new_entry,
  ...
  for (int row = 0; row < p_entry.num_rows; row++) {
    p_entry.values[row * p_entry.stride] =
      z_entry.values[row * z_entry.stride] + beta * p_entry.values[row * p_entry.stride];
  }
}

template<typename ValueType>
void update_x_and_r(const batch::multi_vector::batch_item<const ValueType>& rho_old_entry,
  ...
  for (int row = 0; row < r_entry.num_rows; row++) {
    x_entry.values[row * x_entry.stride] += temp * p_entry.values[row * p_entry.stride];
    r_entry.values[row * r_entry.stride] -= temp * Ap_entry.values[row * Ap_entry.stride];
  }
}

template<typename StopType, typename PrecType, typename LogType, typename BatchMatrixType, typename ValueType>
void batch_entry_cg_impl(const kernels::batch_cg::settings<remove_complex<ValueType>>& settings,
                         LogType logger,
                         PrecType prec,
  ...
  const auto A_entry = extract_batch_item(a, batch_item_id, cpu_kernel{});
  const auto b_entry = b;
  const auto x_entry = x;

  const batch::multi_vector::batch_item<ValueType> r_entry{r, num_rhs, num_rows, num_rhs};
  const batch::multi_vector::batch_item<ValueType> z_entry{z, num_rhs, num_rows, num_rhs};
  const batch::multi_vector::batch_item<ValueType> p_entry{p, num_rhs, num_rows, num_rhs};
  const batch::multi_vector::batch_item<ValueType> Ap_entry{Ap, num_rhs, num_rows, num_rhs};
  ...
  const batch::multi_vector::batch_item<ValueType> rhs_norms_entry{norms_rhs, num_rhs, 1, num_rhs};
  const batch::multi_vector::batch_item<ValueType> res_norms_entry{norms_res, num_rhs, 1, num_rhs};
  ...
  // p = z = Ap = 0
  // rho_old = 1, rho_new = 0
  initialize(A_entry, batch::to_const(b_entry), batch::to_const(x_entry), rho_old_entry, rho_new_entry, r_entry,
             p_entry, z_entry, Ap_entry, rhs_norms_entry);
  ...
  while (true) {
    ...
    // rho_new =  < r , z > = (r)' * (z)
    compute_conj_dot_product_kernel<ValueType>(batch::to_const(r_entry), batch::to_const(z_entry), rho_new_entry);
    ++iter;
    // use implicit residual norms
    res_norms_entry.values[0] = sqrt(abs(rho_new_entry.values[0]));

    if (iter >= settings.max_iterations || stop.check_converged(res_norms_entry.values)) {
    ...
    // p = z + beta * p
    update_p(batch::to_const(rho_new_entry), batch::to_const(rho_old_entry), batch::to_const(z_entry), p_entry);
    ...
    // x = x + temp * p
    // r = r - temp * Ap
    update_x_and_r(batch::to_const(rho_new_entry), batch::to_const(p_entry), batch::to_const(Ap_entry), alpha_entry,
                   x_entry, r_entry);
...
void apply(std::shared_ptr<const DefaultExecutor> exec,
  ...
  auto local_size_bytes =
    static_cast<size_type>(kernels::batch_cg::local_memory_requirement<ValueType>(num_rows, num_rhs));
  ...
  for (int64 batch_id = 0; batch_id < num_batch_items; batch_id++) {
    batch_single_kernels::batch_cg::batch_entry_cg_impl<batch_stop::SimpleRelResidual<ValueType>>(
      options, logger, prec, mat, batch::extract_batch_item(b, batch_id, cpu_kernel{}),
      batch::extract_batch_item(x, batch_id, cpu_kernel{}), batch_id, local_space.get_data());
  }
}
