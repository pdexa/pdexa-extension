...
inline void batch_entry_bicgstab_impl(const kernels::batch_bicgstab::settings<remove_complex<ValueType>>& settings,
  ...
  const auto A_entry = batch::extract_batch_item(a, batch_item_id, cpu_kernel{});
  ...
  auto prec_entry = batch::generate_batch_item(prec, A_entry, reinterpret_cast<char*>(prec_work), batch_item_id, cpu_kernel{});
  ...
  for (iter = 0; iter < settings.max_iterations; iter++) {
    ...
    // p_hat = precond * p
    simple_apply(prec_entry, batch::to_const(p_entry), p_hat_entry);
    ...
    // s_hat = precond * s
    simple_apply(prec_entry, batch::to_const(s_entry), s_hat_entry);
...
void apply(std::shared_ptr<const DefaultExecutor> exec,
  ...
  for (int64 batch_id = 0; batch_id < num_batch_items; batch_id++) {
    batch_single_kernels::batch_entry_bicgstab_impl<batch_stop::SimpleAbsResidual<ValueType>>(
      options, logger, prec, mat, batch::extract_batch_item(b, batch_id, cpu_kernel{}), batch::extract_batch_item(x, batch_id, cpu_kernel{}),
