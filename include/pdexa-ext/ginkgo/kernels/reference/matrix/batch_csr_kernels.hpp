...
inline void simple_apply_impl(const batch::matrix::csr::batch_item<const ValueType, IndexType> a,
  ...
  for (int row = 0; row < a.num_rows; ++row) {
    ...
    for (auto k = a.row_ptrs[row]; k < a.row_ptrs[row + 1]; ++k) {
      ...
      for (int j = 0; j < b.num_rhs; ++j) {
        c.values[row * c.stride + j] += val * b.values[col * b.stride + j];
      }
    }
  }
}
...
template<typename ValueType, typename IndexType>
inline void advanced_apply_impl(const ValueType alpha,
...
template<typename ValueType, typename IndexType>
inline void scale(const ValueType *const col_scale,
...
template<typename ValueType, typename IndexType>
inline void add_scaled_identity(const ValueType alpha,
                                const ValueType beta,
                                const batch::matrix::csr::batch_item<ValueType, IndexType> &mat) {
  for (int row = 0; row < mat.num_rows; row++) {
