...
#include "../preconditioner/batch_identity.hpp"
...
template<typename ValueType>
inline void update_x_and_r(const batch::multi_vector::batch_item<const ValueType>& rho_old_entry,
...
void batch_entry_cg_impl(const kernels::batch_cg::settings<remove_complex<ValueType>>& settings,
  ...
  // rho_old = 1, rho_new = 0
  initialize(A_entry,
             batch::to_const(b_entry),
             batch::to_const(x_entry),
             rho_old_entry,
             rho_new_entry,
             r_entry,
             p_entry,
  ...
  while (true) {
    ...
    // x = x + temp * p
    // r = r - temp * Ap
    update_x_and_r(batch::to_const(rho_new_entry),
                   batch::to_const(p_entry),
                   batch::to_const(Ap_entry),
                   alpha_entry,
                   x_entry,
                   r_entry);
