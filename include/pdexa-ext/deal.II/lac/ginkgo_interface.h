...
create_vector(std::shared_ptr<const gko::Executor> exec, ArrayView<ValueType, MemorySpace> &array) {
  std::shared_ptr<const gko::Executor> array_exec = gko::ext::kokkos::get_executor(
...
create_vector(std::shared_ptr<const gko::Executor> exec, const ArrayView<ValueType, MemorySpace> &array) {
  ...
  gko::dim<2> size = {static_cast<gko::size_type>(array.size()), 1};
  return gko::matrix::Dense<std::decay_t<ValueType>>::create_const(exec, size,
                                                                   gko::make_const_array_view(array_exec, size[0],
                                                                                              array.data()), 1);
}
...
void run_with_device_exec(std::shared_ptr<const gko::Executor> exec, F &&f) {
  if (auto p = std::dynamic_pointer_cast<const gko::CudaExecutor>(exec)) {
    ...
    f(std::move(p));
  } else if (auto p = std::dynamic_pointer_cast<const gko::DpcppExecutor>(exec)) {
    f(std::move(p));
  } else {
    Assert(false, ExcMessage(" encountered unknown device Executor type "));
  }
}

template<typename Matrix, typename DealValueType, typename... ExtraArgs>
std::unique_ptr<Matrix>
create_matrix(std::shared_ptr<const gko::Executor> exec, const SparseMatrix <DealValueType> &deal_csr,
              ExtraArgs &&...args) {
...
enum class csr_strategy {
  exec_based, classical, merge_path, sparselib, load_balance, automatical
};

template<typename ValueType, typename IndexType, typename DealValueType>
std::unique_ptr<gko::matrix::Csr<ValueType, IndexType>>
create_csr_matrix(std::shared_ptr<const gko::Executor> exec, const SparseMatrix<DealValueType> &deal_csr,
  ...
  if (strategy == csr_strategy::exec_based) {
    strategy = std::dynamic_pointer_cast<const gko::ReferenceExecutor>(exec) ||
               std::dynamic_pointer_cast<const gko::OmpExecutor>(exec) ? csr_strategy::classical
...
namespace detail {
class GinkgoPayload : public dealii::internal::LinearOperatorImplementation::EmptyPayload {
};

template<typename ValueType, typename MemorySpace>
ArrayView <ValueType, MemorySpace> create_array_view(LinearAlgebra::distributed::Vector <ValueType, MemorySpace> &v) {
...
template<typename DomainValueType   = double, typename RangeValueType    = double, typename DomainMemorySpace = MemorySpace::Host, typename RangeMemorySpace  = MemorySpace::Host>
dealii::LinearOperator<dealii::LinearAlgebra::distributed::Vector<DomainValueType, DomainMemorySpace>, dealii::LinearAlgebra::distributed::Vector<RangeValueType, RangeMemorySpace>, detail::GinkgoPayload>
linear_operator(const std::shared_ptr<gko::LinOp> &gko_op) {
  using Domain = dealii::LinearAlgebra::distributed::Vector<DomainValueType, DomainMemorySpace>;
  using Range = dealii::LinearAlgebra::distributed::Vector<RangeValueType, RangeMemorySpace>;
  ...
  using LinearOperator = dealii::LinearOperator<Domain, Range, detail::GinkgoPayload>;
