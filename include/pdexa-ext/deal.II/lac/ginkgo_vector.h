...
public:
   ...
   *       executors accordingly.
   */
  Vector &operator=(Vector &&other) noexcept(false);

  /**
   * Creates a vector that is a view on a
   * dealii::LinearAlgebra::ReadWriteVector.
   */
  static std::unique_ptr<Vector> create_view(std::shared_ptr<const gko::Executor> exec,
   ...
   */
  Number operator()(const size_type i) const noexcept;

  /**
   * Mutable access the element @p i of the vector.
   * @warning Depending on the executor, this might access device memory.
   */
  Number &operator()(const size_type i) noexcept;
  ...
  std::unique_ptr<const ginkgo_type> get_gko_object() const noexcept;

  std::unique_ptr<ginkgo_type> get_gko_object() noexcept;
   ...
   */
  void add(const Number a, const Vector &V);
   ...
   */
  void add(const Number a, const Vector &V, const Number b, const Vector &W);
   ...
   * @copydoc add(coconst size_type , const size_type *, const Number *)
   */
  void add(std::vector<unsigned int> &indices, std::vector<Number> &values);
   ...
   */
  void sadd(const Number s, const Number a, const Vector &V);
   ...
   */
  void equ(const Number a, const Vector &V);
   ...
   */
  Number add_and_dot(const Number a, const Vector &V, const Vector &W);
   ...
   */
  void compress(const VectorOperation::values = VectorOperation::insert) const {}
  ...
  void extract_subvector_to(const ArrayView<const size_type> &indices,
