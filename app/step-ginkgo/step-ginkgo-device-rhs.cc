...
std::shared_ptr<gko::Executor> create_executor(MemorySpace) {
  ...
  if (std::is_same<kokkos_space, Kokkos::HIP>::value) {
    return gko::HipExecutor::create(Kokkos::device_id(), create_default_host_executor());
  }
#endif
...
void StepGinkgo<dim>::setup_system() {
  ...
  system_rhs = vec{
      gko::matrix::Dense<double>::create(exec, gko::dim<2>{static_cast<gko::size_type>(system_rhs_d.size()), 1},
                                         gko::make_array_view(exec, system_rhs_d.size(), system_rhs_d.get_values()),
                                         1)};
...
struct functor {
  ...
  void operator()(const unsigned int cell, const typename CUDAWrappers::MatrixFree<dim, Number>::Data *gpu_data,
...
void StepGinkgo<dim>::assemble_rhs() {
  ...
  mf.reinit(dof_handler, constraints, quadrature_formula,
            {dealii::UpdateFlags::update_values | dealii::UpdateFlags::update_gradients |
             dealii::UpdateFlags::update_JxW_values | dealii::UpdateFlags::update_quadrature_points});
