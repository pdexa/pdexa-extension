  ...
  : public gko::EnableLinOp<MatrixFreeOperator<dim, Number>>
{
  ...
  using VectorTypeCeed = typename OperatorCeed<dim, Number>::VectorTypeCeed; 
...
public:
        ...
        std::make_shared<dealii::MappingQ<dim>>(dof_handler.get_fe().degree))
  {
    ...
    operator_ceed = std::make_shared<OperatorCeed<dim, Number>>(*mapping,
                                                                dof_handler,
                                                                *constraints,
                                                                dealii::QGauss<dim>(dof_handler.get_fe().degree + 1),
                                                                libCEED_resource);
  }
  ...
  apply_impl(const gko::LinOp *b, gko::LinOp *x) const override
  {
    ...
    using VectorTypeCeed = typename OperatorCeed<dim, Number>::VectorTypeCeed; 
    VectorTypeCeed src_ceed(operator_ceed->ceed, b_dense);
    VectorTypeCeed dst_ceed(operator_ceed->ceed, x_dense);

    // apply operator
    CeedOperatorApply(operator_ceed->op_apply, src_ceed(), dst_ceed(), CEED_REQUEST_IMMEDIATE);
  }
             ...
             gko::LinOp       *x) const override
  {
    ...
    VectorTypeCeed src_ceed(operator_ceed->ceed, b_dense);
    VectorTypeCeed dst_ceed(operator_ceed->ceed, x_dense);
    auto tmp = gko::matrix::Dense<Number>::create_with_config_of(x_dense);
    VectorTypeCeed tmp_ceed(operator_ceed->ceed, tmp.get());

    // apply operator
    CeedOperatorApply(operator_ceed->op_apply, src_ceed(), tmp_ceed(), CEED_REQUEST_IMMEDIATE);

    x_dense->scale(beta);
    x_dense->add_scaled(alpha, tmp);
  }
...
private:
  ...
  std::shared_ptr<OperatorCeed<dim, Number>> operator_ceed;
};
