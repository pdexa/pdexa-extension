...
namespace NavierStokes
{
        ...
        for (const unsigned int q : eval_u_minus.quadrature_point_indices())
          {
            ...
            const auto p_jump_normal = normal * (p_minus - p_plus);
        ...
        for (const unsigned int q : eval_u_minus.quadrature_point_indices())
          {
            ...
            const auto speed_normal =
              0.5 * (normal * (eval_u_minus.get_value(q) + u_plus));
  ...
  test(const unsigned int degree)
  {
    ...
    const double                    end_time = 6. * time_step;
    ...
    while (time <= end_time)
      {
        ...
        if (time_step_number > 2)
        ...
        else
          {
            ...
            exact_pressure.set_time(time + time_step);
