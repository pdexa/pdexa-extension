        ...
        const unsigned int n_refinements_time)
{
  ...
  const unsigned int bdf_order_p = 4;
  const unsigned int bdf_order_c = 4;
  const unsigned int bdf_order_traction = 4;
  ...
  momentum_op.initialize_dof_vector(vec_phi, dof_no_p);
  ...
  PressureOperator<dim, double> pressure_op, pressure_op_wo_leray;
  ...
  pressure_op_wo_leray.reinit(momentum_op.get_matrix_free(), bdf_order, time_step, false, use_traction_boundary_condition_for_PPE);
  pressure_op_wo_leray.set_body_force_factory([=]() {
    return std::make_unique<AnalyticalRHS<dim>>(u_x_max, viscosity);
  });
  pressure_op_wo_leray.set_viscosity(viscosity);
  pressure_op_wo_leray.set_dirichletBC_pressure_factory ([=]() {
    return std::make_unique<AnalyticalSolutionPressure<dim>>(u_x_max, viscosity);
  });
  pressure_op_wo_leray.set_DirichletBC_velocity_factory ([=]() {
    return std::make_unique<AnalyticalSolutionVelocity<dim>>(u_x_max, viscosity);
  });
...
MultigridPreconditioner<dim, Number, Number> precondition_hmg_wo_leray(pressure_op_wo_leray,
                                                              mapping.get_degree(),
                                                              time_step,
                                                              bdf_order,
                                                              use_hmg,
  ...
  std::size_t n_momentum_iterations = 0, n_pressure_iterations = 0, n_leray_iterations = 0, n_inner_iterations = 0;
  ...
  while (current_time <= end_time)
    {
      ...
      momentum_op.set_time(current_time);
      pressure_op.set_time(current_time);
      pressure_op_wo_leray.set_time(current_time);

      while(keep_iterating && inner_iterations < 1000)
      {
        // do the pressure
        vec_p_rhs = 0.;
        pressure_op_wo_leray.compute_convective_rhs(vec_p_rhs, vec_u);
        ...
        vec_p_rhs_n = 0.;
        if(use_traction_boundary_condition_for_PPE)
          pressure_op_wo_leray.compute_rhs(vec_p_rhs_n, vec_vorticity, vec_u);
        else
          pressure_op_wo_leray.compute_rhs(vec_p_rhs_n, vec_vorticity);
        vec_p_rhs.add(1.0, vec_p_rhs_n);

        if (!use_neumann_boundary)
        VectorTools::subtract_mean_value(vec_p_rhs);

      const unsigned int iteration_count_inner =
        precondition_hmg_wo_leray.solve(pressure_op_wo_leray, vec_p, vec_p_rhs);
        if (!use_neumann_boundary)
          VectorTools::subtract_mean_value(vec_p);
        if (write_output)
          pcout << "Pressure solver: " << iteration_count_inner << " iterations" << std::endl;
        n_pressure_iterations += iteration_count_inner;

        // now do the momentum with Leray correction for the acceleration term
        // add the Leray terms to vec_phi combined with the pressure
        vec_phi = 0.;
        vec_phi.add(1.0, vec_p);
        if (use_leray_projection)
          for (unsigned int i = 0; i < bdf.get_order(); ++i)
              vec_phi.add(-bdf.get_alpha(i) / time_step, vec_phi_old[i]);

        vec_u_rhs = 0.;
        momentum_op.rhs(vec_u_rhs, vec_u_deriv, vec_u, vec_phi);
        solver_mom.solve(momentum_op, vec_u, vec_u_rhs, inverse_mass);
        if (write_output)
          pcout << "Momentum solver: " << control_mom.last_step() << " iterations"
                << std::endl;
        ...
        speed_extrapolated.add(-1.0, vec_u);
        if(speed_extrapolated.l2_norm() < 1e-6)
      ...
      n_inner_iterations += inner_iterations;
      ...
      {
        vec_div_u = 0.;
        pressure_op_wo_leray.compute_divergence(vec_div_u, vec_u);
        const unsigned int iteration_count_leray =
          precondition_hmg_wo_leray.solve(pressure_op_wo_leray, vec_phi, vec_div_u);

        n_leray_iterations += iteration_count_leray;
      }
  ...
  pcout << "Average iteration count in " << n_performed_steps
        << " time steps for pressure / momentum / leray: "
        << static_cast<double>(n_pressure_iterations) / n_performed_steps << " / "
        << static_cast<double>(n_momentum_iterations) / n_performed_steps << " / "
        << static_cast<double>(n_leray_iterations) / n_performed_steps << " in "
        << n_inner_iterations << " inner iterations" << std::endl;
  pcout << std::endl;
}
