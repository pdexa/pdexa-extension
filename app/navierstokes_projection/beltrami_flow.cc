...
public:
         ...
         const unsigned int     bdf_order_in)
  {
    ...
    penalty_factor =
      10.0 * (dof_handler_u.get_fe().degree + 1) * (dof_handler_u.get_fe().degree);
    {
...
public:
        ...
        const VectorTypeSystem &vec_u_rhs)
  {
    ...
    if(use_amg_as_coarse_grid_solver_vel)
    {
      ...
      mg_coarse =  
        std::make_unique<MGCoarseAMG<VectorType>>(precondition_amg, false);
    }
    else
    {
    ...
    SolverControl                 control(100000, 1e-9 * vec_u_rhs.l2_norm());
...
public:
  ...
  operator()(const unsigned int /*level*/,
             ...
             const VectorType &src) const final
  {
    ReductionControl coarse_grid_solver_control(10000, 1e-8, 1e-4, false, false);
        ...
        const unsigned int n_refinements_time)
{
  ...
  GridGenerator::hyper_cube(tria, -L, L);
      ...
      if (write_output && time_step_number % output_interval == 0)
        {
          ...
          VectorTools::interpolate(mapping,
                                            ...
                                            exact_pressure,
                                            vec_p_norm);
          vec_p.add( vec_p_norm.mean_value());
          ...
          const double pressure_norm =
            VectorTools::compute_global_error(tria, norm_per_cell, VectorTools::L2_norm);
          pcout << "L2 norm velocity/pressure: " << velocity_error/velocity_norm << " "
          ...
          data_out.add_data_vector(dof_handler_u, vec_u_norm, "velocity_analytical");
          data_out.add_data_vector(dof_handler_p, vec_p_norm, "pressure_analytical");
          ...
          const std::string filename =
            "solution-beltrami-" + std::to_string(time_step_number) + ".vtu";
  ...
  const double velocity_error =
    VectorTools::compute_global_error(tria,
                                      error_per_cell,
                                      VectorTools::L2_norm); // H1_seminorm);
  if(std::abs(vec_p.mean_value()) < 1e-18)
  {
    VectorTools::interpolate(mapping,
                              dof_handler_p,
                              exact_pressure,
