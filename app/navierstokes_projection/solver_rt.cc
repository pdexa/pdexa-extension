...
namespace NavierStokes
{
  ...
  public:
    ...
    value(const dealii::Point<dim> &p, const unsigned int component = 0) const final
    {
      ...
      double result = 0.0;
      if (component == 0)
        result = -u_x_max * std::sin(2.0 * pi * p[1]) /* std::cos(2 * pi * p[0])*/ *
                 std::exp(-4.0 * pi * pi * viscosity * t);
      else if (component == 1)
        result = u_x_max * std::sin(2.0 * pi * p[0]) /* std::cos(2 * pi * p[1])*/ *
                 std::exp(-4.0 * pi * pi * viscosity * t);
      /*
      if (component == 0)
  ...
  public:
    ...
    void
    rhs(VectorType &dst, const VectorType &u_rhs_time_derivative_pressure)
    {
      matrix_free->loop(&PressureOperator::local_rhs,
                        &PressureOperator::local_apply_boundary_face,
                        &PressureOperator::local_apply_boundary_face,
                        this,
                        dst,
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
        {
          ...
          for (const unsigned int q : eval_u.quadrature_point_indices())
            {
              eval_p.submit_value(-eval_u.get_divergence(q), q);
            }
    ...
    while (time <= end_time)
      {
        ...
        VectorTools::integrate_difference(mapping,
                                          ...
                                          exact_velocity,
                                          error_per_cell,
                                          QGaussLobatto<dim>(fe_u.degree + 1),
                                          VectorTools::L2_norm);
        std::cout << "L2 errors: "
