    ...
    for (unsigned int cell = range.first; cell < range.second; ++cell)
      {
        ...
        for (unsigned int q = 0; q < integrator.n_q_points; ++q)
         {
           const auto time_deriv = make_vectorized_array<number>(1.5 / time_step)*integrator.get_value(q);
    ...
    for (unsigned int face = range.first; face < range.second; ++face)
      {
        ...
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          { 
            ...
            speeds_faces(face, q) = make_vectorized_array<number>(0.5)* (integrator_speed_inner.get_value(q) + integrator_speed_outer.get_value(q)) * normal;
    ...
    for (unsigned int face = range.first; face < range.second; ++face)
      {
        ...
        if (matrix_free.get_boundary_id(face) == 0)
        {
          for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          {
            ...
            const auto speed_normal = make_vectorized_array<number>(0.5)* (integrator_speed_inner.get_value(q) + g) * normal;
      ...
      for (unsigned int face = face_range.first; face < face_range.second; face++)
      {
        if (data.get_boundary_id(face) == 0)
        {
          ...
          { 
            ...
            auto u_plus =  make_vectorized_array(1.5 / time_step) * g;
            
            exact_velocity.set_time(time - time_step);
            u_plus -= make_vectorized_array(2.0 / time_step) * evaluate_function(exact_velocity, eval_p_minus.quadrature_point(q));   
            exact_velocity.set_time(time - 2.0 * time_step);
            u_plus -= make_vectorized_array(-0.5 / time_step) * evaluate_function(exact_velocity, eval_p_minus.quadrature_point(q));            
