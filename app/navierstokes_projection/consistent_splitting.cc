...
public:
  ...
  double value(const dealii::Point<dim>& p, const unsigned int component = 0) const final {
  ...
  dealii::Tensor<1, dim, double> gradient(const dealii::Point<dim>& p, const unsigned int component = 0) const final {
    const double t = this->get_time();
    const double pi = dealii::numbers::PI;
...
public:
  ...
  double value(const dealii::Point<dim>& p, const unsigned int /*component*/) const final {
    const double t = this->get_time();
    const double pi = dealii::numbers::PI;

    const double result =
      -std::cos(2. * pi * p[0]) * std::cos(2. * pi * p[1]) * std::exp(-8. * viscosity * pi * pi * t);
...
evaluate_function(const Function<dim>& function, const Point<dim, VectorizedArray<Number>>& p_vectorized) {
  ...
  for (unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
    ...
    for (unsigned int d = 0; d < n_components; ++d) result[d][v] = function.value(p, d);
  }
  return result;
}

template<int dim, typename Number>
VectorizedArray<Number> evaluate_scalar_function(const Function<dim>& function,
...
evaluate_tensor_function(const Function<dim>& function, const Point<dim, VectorizedArray<number>>& p_vectorized) {
  ...
  for (unsigned int v = 0; v < VectorizedArray<number>::size(); ++v) {
    ...
    for (unsigned int d = 0; d < n_components; ++d) {
      ...
      for (unsigned int e = 0; e < dim; ++e) result[d][e][v] = func_eval[e];
    }
  }
  return result;
}
...
struct CurlCompute {
  static typename FEEval::value_type compute(const FEEval& fe_eval, const unsigned int q_point) {
...
public:
  ...
  double get_gamma0() { return gamma0; }
  ...
  unsigned int get_order() { return alpha.size(); }
...
public:
  ...
  void reinit(const Mapping<dim>& mapping,
    ...
    data.mapping_update_flags = (update_gradients | update_JxW_values | update_quadrature_points | update_values);
    ...
    matrix_free.reinit(mapping, std::vector<const DoFHandler<dim>*>{&dof_handler_u, &dof_handler_p},
                       std::vector<const AffineConstraints<double>*>{&dummy, &dummy},
                       std::vector<Quadrature<1>>{{quadrature, quadrature_mass, quadrature_p}}, data);
    ...
    speeds_faces.reinit(matrix_free.n_inner_face_batches() + matrix_free.n_boundary_face_batches(),
                        eval_face.n_q_points);

    penalty_factor = 1.0 * (dof_handler_u.get_fe().degree + 1) * (dof_handler_u.get_fe().degree);
    {
      ...
      const auto quadrature = reference_cells.template get_gauss_type_quadrature<dim>(fe_degree_u + 1);
      dealii::FEValues<dim> fe_values(mapping, fe, quadrature, dealii::update_JxW_values);

      const auto face_quadrature =
        reference_cells.face_reference_cell(0).template get_gauss_type_quadrature<dim - 1>(fe_degree_u + 1);
      ...
      for (unsigned int i = 0; i < n_cells; ++i) {
        for (unsigned int v = 0; v < matrix_free.n_active_entries_per_cell_batch(i); ++v) {
          typename dealii::DoFHandler<dim>::cell_iterator cell = matrix_free.get_cell_iterator(i, v);
          fe_values.reinit(cell);

          // calculate cell volume
          number volume = 0;
          for (unsigned int q = 0; q < quadrature.size(); ++q) { volume += fe_values.JxW(q); }

          // calculate surface area
          number surface_area = 0;
          for (const unsigned int f: cell->face_indices()) {
            fe_face_values.reinit(cell, f);
            const number factor = (cell->at_boundary(f) and not(cell->has_periodic_neighbor(f))) ? 1. : 0.5;
  ...
  virtual void initialize_dof_vector(VectorType& vec, const unsigned int dof_index) const {
    matrix_free.initialize_dof_vector(vec, dof_index);
  }

  virtual void set_viscosity(number viscosity_in) { viscosity = viscosity_in; }

  virtual void set_time(number time_in) { time = time_in; }

  virtual void vmult(VectorViewType& dst, const VectorViewType& src) const {
    this->matrix_free.loop(&MomentumOperator::do_cell_integral_range, &MomentumOperator::do_face_integral_range,
                           &MomentumOperator::do_boundary_integral_range, this, dst, src, true,
                           MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                           MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }

  virtual void rhs(VectorType& dst,
                   const VectorType& velocity_derivative,
                   const VectorType& speed,
                   const VectorType& pressure) const {
    this->matrix_free.loop(&MomentumOperator::do_rhs_cell_integral_range, &MomentumOperator::do_rhs_face_integral_range,
                           &MomentumOperator::do_rhs_boundary_range, this, dst,
                           std::vector<const VectorType*>{&velocity_derivative, &speed, &pressure}, true,
                           MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                           MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }

  void evaluate_vorticity(VectorType& dst, const VectorType& src) const {
    this->matrix_free.cell_loop(&MomentumOperator::local_vorticity_domain, this, dst, src, true);
  ...
  const MatrixFree<dim, number>& get_matrix_free() const { return matrix_free; }

private:
  number get_penalty_factor() const { return penalty_factor; }

  void do_cell_integral_range(const MatrixFree<dim, number>& matrix_free,
                              VectorViewType& dst,
                              const VectorViewType& src,
                              const std::pair<unsigned int, unsigned int>& range) const {
    FEEvaluation<dim, -1, 0, n_components, Number> integrator(matrix_free, dof_no_v, quad_no_v);
    ...
    for (unsigned int cell = range.first; cell < range.second; ++cell) {
      ...
      for (unsigned int q = 0; q < integrator.n_q_points; ++q) {
        ...
        const auto divergence_penalty = penalty_factor_divergence[cell] * integrator.get_divergence(q);
        Tensor<2, dim, VectorizedArray<number>> div_penalty;
        for (unsigned int d = 0; d < dim; ++d)
          for (unsigned int e = 0; e < dim; ++e) div_penalty[d][e] = 0.;

        for (unsigned int d = 0; d < dim; ++d) div_penalty[d][d] = divergence_penalty;
        ...
        if (!use_skew_symmetric_convective_formulation) {
          ...
          integrator.submit_gradient(div_penalty + make_vectorized_array<number>(viscosity) * grad_u, q);
        }
        else {
          ...
          integrator.submit_gradient(
            div_penalty + make_vectorized_array<number>(viscosity) * grad_u + convective_gradient_flux, q);
        }
      }
  ...
  void do_face_integral_range(const MatrixFree<dim, number>& matrix_free,
                              VectorViewType& dst,
                              const VectorViewType& src,
                              const std::pair<unsigned int, unsigned int>& range) const {
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_inner(matrix_free, true, dof_no_v, quad_no_v);
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_outer(matrix_free, false, dof_no_v, quad_no_v);
    ...
    for (unsigned int face = range.first; face < range.second; ++face) {
      ...
      integrator_outer.gather_evaluate(src, EvaluationFlags::values | EvaluationFlags::gradients);

      const VectorizedArray<number> sigma = std::max(integrator_inner.read_cell_data(array_penalty_parameter),
                                                     integrator_outer.read_cell_data(array_penalty_parameter)) *
                                            get_penalty_factor();

      const VectorizedArray<number> cont_pen = 0.5 * (integrator_inner.read_cell_data(penalty_factor_continuity) +
      ...
      for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q) {
        ...
        const Tensor<1, dim, VectorizedArray<number>> solution_average =
          make_vectorized_array<number>(0.5) * (integrator_inner.get_value(q) + integrator_outer.get_value(q));

        const Tensor<1, dim, VectorizedArray<number>> averaged_normal_derivative =
          make_vectorized_array<number>(viscosity) *
          (integrator_inner.get_normal_derivative(q) + integrator_outer.get_normal_derivative(q)) * number(0.5);

        const Tensor<1, dim, VectorizedArray<number>> test_by_value =
          make_vectorized_array<number>(viscosity) * solution_jump * sigma - averaged_normal_derivative;

        const auto speed = speeds_faces(face, q);
        const auto speed_normal = speed * normal;
        const auto convective_flux =
          speed_normal * solution_average + make_vectorized_array<number>(0.5) * std::abs(speed_normal) * solution_jump;

        const auto convective_flux_inner = convective_flux - speed_normal * integrator_inner.get_value(q);
        const auto convective_flux_outer = -convective_flux + speed_normal * integrator_outer.get_value(q);

        const auto continuity_penalty_value = cont_pen * (solution_jump * normal) * normal;

        if (!use_skew_symmetric_convective_formulation) {
          integrator_inner.submit_value(continuity_penalty_value + test_by_value + convective_flux_inner, q);
          integrator_outer.submit_value(-continuity_penalty_value - test_by_value + convective_flux_outer, q);
        }
        else {
          const auto convective_value_flux =
            speed * (solution_average * normal) + 0.5 * (std::abs(speed_normal) * solution_jump);

          integrator_inner.submit_value(
            continuity_penalty_value + test_by_value + 0.5 * convective_flux_inner + 0.5 * convective_value_flux, q);
          integrator_outer.submit_value(
            -continuity_penalty_value - test_by_value + 0.5 * convective_flux_outer - 0.5 * convective_value_flux, q);
        }
      ...
      integrator_inner.integrate_scatter(EvaluationFlags::values | EvaluationFlags::gradients, dst);
      integrator_outer.integrate_scatter(EvaluationFlags::values | EvaluationFlags::gradients, dst);
    }
  }

  void do_boundary_integral_range(const MatrixFree<dim, number>& matrix_free,
                                  VectorViewType& dst,
                                  const VectorViewType& src,
                                  const std::pair<unsigned int, unsigned int>& range) const {
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_inner(matrix_free, true, dof_no_v, quad_no_v);
    ...
    for (unsigned int face = range.first; face < range.second; ++face) {
      ...
      const VectorizedArray<number> cont_pen =
        integrator_inner.read_cell_data(penalty_factor_continuity); // Dirichlet boundary
      if (matrix_free.get_boundary_id(face) == 0) {
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q) {
          ...
          const Tensor<1, dim, VectorizedArray<number>> normal_derivative_inner =
            make_vectorized_array<number>(viscosity) * integrator_inner.get_normal_derivative(q);
          ...
          const auto speed = speeds_faces(face, q);
          const auto speed_normal = speed * normal;
          const auto convective_flux = (std::abs(speed_normal) - speed_normal) * integrator_inner.get_value(q);

          const auto g = evaluate_function(exact_velocity, integrator_inner.quadrature_point(q));
          const auto continuity_penalty_value = 2. * cont_pen * ((integrator_inner.get_value(q) - g) * normal) * normal;
          if (!use_skew_symmetric_convective_formulation) {
            integrator_inner.submit_value(continuity_penalty_value + test_by_value + convective_flux, q);
          }
          else {
            const auto convective_value_flux = (std::abs(speed_normal)) * integrator_inner.get_value(q);
            integrator_inner.submit_value(
              continuity_penalty_value + test_by_value + 0.5 * convective_flux + 0.5 * convective_value_flux, q);
          }
        ...
        for (const unsigned int q: integrator_inner.quadrature_point_indices()) {
          integrator_inner.submit_normal_derivative(Tensor<1, dim, VectorizedArray<number>>(), q);
          if (!use_skew_symmetric_convective_formulation)
            integrator_inner.submit_value(Tensor<1, dim, VectorizedArray<number>>(), q);
          else {
            const auto speed = speeds_faces(face, q);
            const auto convective_flux = speed * (integrator_inner.get_value(q) * integrator_inner.normal_vector(q));
            integrator_inner.submit_value(0.5 * convective_flux, q);
          }
        }
      }
      else
        AssertThrow(false, ExcNotImplemented("Boundary id " + std::to_string(int(matrix_free.get_boundary_id(face))) +
                                             " not known"));
  ...
  void do_rhs_cell_integral_range(const MatrixFree<dim, number>& matrix_free,
                                  VectorType& dst,
                                  const std::vector<const VectorType*>& src,
                                  const std::pair<unsigned int, unsigned int>& range) const {
    FEEvaluation<dim, -1, 0, n_components, Number> integrator(matrix_free, dof_no_v, quad_no_v);
    FEEvaluation<dim, -1, 0, n_components, Number> integrator_speed(matrix_free, dof_no_v, quad_no_v);
    ...
    for (unsigned int cell = range.first; cell < range.second; ++cell) {
      ...
      for (unsigned int q = 0; q < integrator_speed.n_q_points; ++q) {
        ...
        avg_veclocity += integrator_speed.JxW(q) * integrator_speed.get_value(q).norm();
      }
      avg_veclocity /= volume;
      penalty_factor_divergence[cell] =
        penalty_divergence * avg_veclocity * std::exp(std::log(volume) / (number) dim) / ((number) fe_degree_u + 1);
  ...
  void do_rhs_face_integral_range(const MatrixFree<dim, number>& matrix_free,
                                  VectorType& dst,
                                  const std::vector<const VectorType*>& src,
                                  const std::pair<unsigned int, unsigned int>& range) const {
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_inner(matrix_free, true, dof_no_v, quad_no_v);
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_outer(matrix_free, false, dof_no_v, quad_no_v);
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_speed_inner(matrix_free, true, dof_no_v, quad_no_v);
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_speed_outer(matrix_free, false, dof_no_v, quad_no_v);
    FEFaceEvaluation<dim, -1, 0, 1, Number> integrator_inner_p(matrix_free, true, dof_no_p, quad_no_v);
    FEFaceEvaluation<dim, -1, 0, 1, Number> integrator_outer_p(matrix_free, false, dof_no_p, quad_no_v);
    ...
    for (unsigned int face = range.first; face < range.second; ++face) {
      ...
      for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q) {
        ...
        const Tensor<1, dim, VectorizedArray<number>> p_jump =
          number(0.5) * (integrator_inner_p.get_value(q) - integrator_outer_p.get_value(q)) * normal;
  ...
  void do_rhs_boundary_range(const MatrixFree<dim, number>& matrix_free,
                             VectorType& dst,
                             const std::vector<const VectorType*>& src,
                             const std::pair<unsigned int, unsigned int>& range) const {
    FEFaceEvaluation<dim, -1, 0, n_components, Number> integrator_inner(matrix_free, true, dof_no_v, quad_no_v);
      ...
      if (matrix_free.get_boundary_id(face) == 0) {
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q) {
          ...
          if (use_extrapolated_velocity) {
            const auto u_plus_m = evaluate_function(exact_velocity_m, integrator_inner.quadrature_point(q));
            const auto u_plus_m2 = evaluate_function(exact_velocity_m2, integrator_inner.quadrature_point(q));
            auto extrapolated_velocity = 2.0 * u_plus_m - u_plus_m2;
            speed = 0.5 * (integrator_speed_inner.get_value(q) + extrapolated_velocity);
          }
          else { speed = make_vectorized_array<number>(0.5) * (integrator_speed_inner.get_value(q) + g); }
          ...
          const auto value_flux = make_vectorized_array<number>(2.0 * viscosity) * sigma * g;
          const auto gradient_flux = make_vectorized_array<number>(viscosity) * g;

          const Tensor<1, dim, VectorizedArray<number>> p = 0. * integrator_inner.normal_vector(q);
          ...
          if (!use_skew_symmetric_convective_formulation) {
            ...
            const auto convective_value_flux = -speed * (g * normal) + std::abs(speed_normal) * g;
            integrator_inner.submit_value(value_flux - p + 0.5 * convective_flux + 0.5 * convective_value_flux, q);
          }
        }
      }
      else if (matrix_free.get_boundary_id(face) == 1) {
        for (const unsigned int q: integrator_inner.quadrature_point_indices()) {
          ...
          const auto grad_g = evaluate_tensor_function(exact_velocity, integrator_inner.quadrature_point(q));
          const auto h_u = make_vectorized_array(viscosity) * grad_g * normal;

          const auto p_plus = evaluate_scalar_function(exact_pressure, integrator_inner.quadrature_point(q));
  ...
  void local_vorticity_domain(const MatrixFree<dim, number>& data,
                              VectorType& dst,
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for (unsigned int q = 0; q < eval_u.n_q_points; ++q) {
        if constexpr (dim == 2) {
          ...
          for (unsigned int d = 0; d < dim; ++d) omega_vector[d] = 0.;
          omega_vector[0] = omega[0];
          eval_u.submit_value(omega_vector, q);
        }
        else if constexpr (dim == 3) { eval_u.submit_value(eval_u.get_curl(q), q); }
      }
  ...
  mutable dealii::AlignedVector<dealii::VectorizedArray<Number>> penalty_factor_divergence;
  mutable dealii::AlignedVector<dealii::VectorizedArray<Number>> penalty_factor_continuity;
...
public:
  ...
  void reinit(const MatrixFree<dim, number>& matrix_free, const unsigned int bdf_order_in) {
    bdf_order = bdf_order_in;
    ...
    const double penalty_factor = 1.0 * (fe_degree + 1) * (fe_degree);
    {
      ...
      const dealii::FiniteElement<dim>& fe = matrix_free.get_dof_handler(dof_no_p).get_fe();
      const auto reference_cells = matrix_free.get_dof_handler(dof_no_p).get_fe().reference_cell();
      MappingQ1<dim> mapping;

      const auto quadrature = reference_cells.template get_gauss_type_quadrature<dim>(fe_degree + 1);
      dealii::FEValues<dim> fe_values(mapping, fe, quadrature, dealii::update_JxW_values);

      const auto face_quadrature =
        reference_cells.face_reference_cell(0).template get_gauss_type_quadrature<dim - 1>(fe_degree + 1);
      dealii::FEFaceValues<dim> fe_face_values(mapping, fe, face_quadrature, dealii::update_JxW_values);

      for (unsigned int i = 0; i < n_cells; ++i) {
        for (unsigned int v = 0; v < matrix_free.n_active_entries_per_cell_batch(i); ++v) {
          typename dealii::DoFHandler<dim>::cell_iterator cell = matrix_free.get_cell_iterator(i, v, dof_no_p);
          fe_values.reinit(cell);
  ...
  void set_time(const double time) { this->time = time; }
  ...
  void vmult(VectorViewType& dst, const VectorViewType& src) const {
    matrix_free->loop(&PressureOperator::local_apply_domain, &PressureOperator::local_apply_inner_face,
                      &PressureOperator::local_apply_boundary_face, this, dst, src, true,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }

  void compute_rhs(VectorType& dst, const VectorType& vorticity) {
    matrix_free->loop(&PressureOperator::local_rhs_domain, &PressureOperator::local_rhs_inner_face,
                      &PressureOperator::local_rhs_boundary_face, this, dst, vorticity, true,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }

  void compute_convective_rhs(VectorType& dst, const VectorType& velocity) {
    matrix_free->loop(&PressureOperator::local_convective_domain, &PressureOperator::local_convective_inner_face,
                      &PressureOperator::local_convective_boundary_face, this, dst, velocity, true,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }

  void compute_divergence(VectorType& dst, const VectorType& velocity) {
    matrix_free->loop(&PressureOperator::local_divergence_domain, &PressureOperator::local_divergence_inner_face,
                      &PressureOperator::local_divergence_boundary_face, this, dst, velocity, true,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }

  void set_time_step(const double t) { time_step = t; }
...
private:
  ...
  void local_apply_domain(const MatrixFree<dim, number>& data,
                          VectorViewType& dst,
                          const VectorViewType& src,
                          const std::pair<unsigned int, unsigned int>& cell_range) const {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for (const unsigned int q: eval.quadrature_point_indices()) eval.submit_gradient(eval.get_gradient(q), q);
  ...
  void local_apply_inner_face(const MatrixFree<dim, number>& data,
                              VectorViewType& dst,
                              const VectorViewType& src,
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      ...
      const VectorizedArray<number> penalty_factor =
        std::max(eval_minus.read_cell_data(array_penalty_parameter), eval_plus.read_cell_data(array_penalty_parameter));
      ...
      for (const unsigned int q: eval_minus.quadrature_point_indices()) {
        ...
        const auto viscous_gradient_flux = make_vectorized_array<number>(0.5) * (u_plus - u_minus);
  ...
  void local_apply_boundary_face(const MatrixFree<dim, number>& data,
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      ...
      const VectorizedArray<number> penalty_factor = eval_minus.read_cell_data(array_penalty_parameter);
      ...
      if (data.get_boundary_id(face) == 0) {
        ...
        for (const unsigned int q: eval_minus.quadrature_point_indices()) {
          ...
          const auto viscous_value_flux = 2.0 * penalty_factor * u_minus - eval_minus.get_normal_derivative(q);
        ...
        AssertThrow(false,
                    ExcNotImplemented("Boundary id " + std::to_string(int(data.get_boundary_id(face))) + " not known"));

      eval_minus.integrate_scatter(EvaluationFlags::values | EvaluationFlags::gradients, dst);
    }
  }

  void local_rhs_domain(const MatrixFree<dim, number>& data,
                        VectorType& dst,
                        const VectorType&,
                        const std::pair<unsigned int, unsigned int>& cell_range) const {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      // multiply by nabla v^h(x) and sum
      eval_p.integrate_scatter(EvaluationFlags::values | EvaluationFlags::gradients, dst);
    }
  }

  void local_rhs_inner_face(const MatrixFree<dim, number>& data,
                            VectorType& dst,
                            const VectorType&,
                            const std::pair<unsigned int, unsigned int>& face_range) const {
  ...
  void local_rhs_boundary_face(const MatrixFree<dim, number>& data,
                               VectorType& dst,
                               const VectorType& src,
                               const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, -1, 0, 1, number> eval_p_minus(data, true, 1, 1);
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      if (data.get_boundary_id(face) == 0) {
        ...
        for (const unsigned int q: eval_p_minus.quadrature_point_indices()) {
          ...
          exact_velocity.set_time(time);
          const auto g = evaluate_function(exact_velocity, eval_p_minus.quadrature_point(q));
          ...
          Tensor<1, dim, VectorizedArray<number>> curl_omega =
            CurlCompute<dim, FEFaceEvaluation<dim, -1, 0, dim, number>>::compute(eval_vorticity, q);

          if (use_analytical_curl) { curl_omega = make_vectorized_array<number>(4.0 * numbers::PI * numbers::PI) * g; }
        ...
        for (const unsigned int q: eval_p_minus.quadrature_point_indices()) {
          ...
          const auto g_p = evaluate_scalar_function(exact_pressure, eval_p_minus.quadrature_point(q));

          const VectorizedArray<number> penalty_factor = eval_p_minus.read_cell_data(array_penalty_parameter);
  ...
  void local_convective_domain(const MatrixFree<dim, number>& data,
                               VectorType& dst,
                               const VectorType& src,
                               const std::pair<unsigned int, unsigned int>& cell_range) const {
  ...
  void local_convective_inner_face(const MatrixFree<dim, number>& data,
                                   VectorType& dst,
                                   const VectorType& src,
                                   const std::pair<unsigned int, unsigned int>& face_range) const {
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      ...
      eval_u_plus.gather_evaluate(src, EvaluationFlags::values | EvaluationFlags::gradients);
      ...
      for (const unsigned int q: eval_p_minus.quadrature_point_indices()) {
        ...
        if (use_pressure_convective_upwind_flux) {
          ...
          const auto value_flux = 0.5 * (u_minus_grad + u_plus_grad) * 0.5 * (u_minus + u_plus);
  ...
  void local_convective_boundary_face(const MatrixFree<dim, number>& data,
                                      VectorType& dst,
                                      const VectorType& src,
                                      const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, -1, 0, 1, number> eval_p_minus(data, true, dof_no_p, 1);
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      if (data.get_boundary_id(face) == 0) {
        ...
        for (const unsigned int q: eval_p_minus.quadrature_point_indices()) {
          ...
          exact_velocity.set_time(time);
          const auto grad_u = evaluate_tensor_function(exact_velocity, eval_p_minus.quadrature_point(q));
  ...
  void local_divergence_domain(const MatrixFree<dim, number>& data,
                               VectorType& dst,
                               const VectorType& src,
                               const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, -1, 0, 1, number> eval_p(data, dof_no_p, 1);
  ...
  void local_divergence_inner_face(const MatrixFree<dim, number>& data,
                                   VectorType& dst,
                                   const VectorType& src,
                                   const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, -1, 0, 1, number> eval_p_minus(data, true, dof_no_p, 1);
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      ...
      for (const unsigned int q: eval_p_minus.quadrature_point_indices()) {
        ...
        const auto normal = eval_p_minus.normal_vector(q);
        const auto div_factor = -0.5 * (eval_u_minus.get_value(q) - eval_u_plus.get_value(q)) * normal;
  ...
  void local_divergence_boundary_face(const MatrixFree<dim, number>& data,
                                      VectorType& dst,
                                      const VectorType& src,
                                      const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, -1, 0, 1, number> eval_p_minus(data, true, dof_no_p, 1);
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++) {
      if (data.get_boundary_id(face) == 0) {
        ...
        for (const unsigned int q: eval_p_minus.quadrature_point_indices()) { eval_p_minus.submit_value({}, q); }
...
template<int dim, typename Number>
void do_test(const unsigned int fe_degree, const unsigned int n_refinements, const unsigned int n_refinements_time) {
  ...
  pcout << "Solving with " << fe_u.get_name() << " x " << fe_p.get_name() << " element" << std::endl;
  pcout << "number of degrees of freedom: " << dof_handler_u.n_dofs() << " + " << dof_handler_p.n_dofs() << std::endl;
  ...
  for (unsigned int i = 1; i < n_refinements_time; ++i) time_step_size *= 0.5;
  ...
  LinearAlgebra::distributed::Vector<Number, memory_space> vec_u, vec_u_deriv, vec_u_rhs, vec_p, vec_u_norm,
    speed_extrapolated, vec_vorticity, vec_p_rhs, vec_p_rhs_n, vec_p_norm, vec_div_u;
  ...
  for (auto& vec: vec_u_old) momentum_op.initialize_dof_vector(vec, dof_no_v);
  ...
  while (current_time <= end_time) {
    ...
    for (unsigned int i = 0; i < bdf_p.get_order(); ++i) speed_extrapolated.add(bdf_p.get_beta(i), vec_u_old[i]);
    ...
    if (!use_neumann_boundary) VectorTools::subtract_mean_value(vec_p_rhs);
    vec_p = 0.;
    ...
    for (unsigned int i = bdf.get_order() - 1; i != 0; --i) { std::swap(vec_u_old[i], vec_u_old[i - 1]); }
    ...
    if (write_output) {
      ...
      VectorTools::integrate_difference(mapping, dof_handler_u, vec_u_old[0], exact_velocity, error_per_cell,
                                        QGauss<dim>(fe_u.degree + 3), VectorTools::L2_norm);
      const double velocity_error = VectorTools::compute_global_error(tria, error_per_cell, VectorTools::L2_norm);

      VectorTools::integrate_difference(mapping, dof_handler_p, vec_p, exact_pressure, error_per_cell,
                                        QGauss<dim>(fe_p.degree + 3), VectorTools::L2_norm);
      const double pressure_error = VectorTools::compute_global_error(tria, error_per_cell, VectorTools::L2_norm);

      vec_u_norm = 0.;
      VectorTools::integrate_difference(mapping, dof_handler_u, vec_u_norm, exact_velocity, norm_per_cell,
                                        QGauss<dim>(fe_u.degree + 3), VectorTools::L2_norm);
      const double velocity_norm = VectorTools::compute_global_error(tria, norm_per_cell, VectorTools::L2_norm);

      vec_p_norm = 0.;
      VectorTools::integrate_difference(mapping, dof_handler_p, vec_p_norm, exact_pressure, norm_per_cell,
                                        QGauss<dim>(fe_p.degree + 3), VectorTools::L2_norm);
      const double pressure_norm = VectorTools::compute_global_error(tria, norm_per_cell, VectorTools::L2_norm);

      pcout << "L2 error velocity/pressure: " << velocity_error / velocity_norm << " " << pressure_error / pressure_norm
            << std::endl;
      pcout << std::endl;
      ...
      const std::string filename = "solution-L2-" + std::to_string(time_step_number) + ".vtu";
  ...
  const double velocity_error = VectorTools::compute_global_error(tria, error_per_cell,
                                                                  VectorTools::L2_norm); // H1_seminorm);
  ...
  VectorTools::integrate_difference(mapping, dof_handler_u, vec_u_norm, exact_velocity, norm_per_cell,
                                    QGauss<dim>(fe_u.degree + 3),
                                    VectorTools::L2_norm); // H1_seminorm);
  const double velocity_norm = VectorTools::compute_global_error(tria, norm_per_cell,
                                                                 VectorTools::L2_norm); // H1_seminorm);
