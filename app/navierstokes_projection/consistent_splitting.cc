...
template <int dim>
  ...
  public:
    ...
    value(const dealii::Point<dim> &p, const unsigned int /*component*/) const final
    {
        ...
        const double result = -std::cos(2.*pi*p[0])*std::cos(2.*pi*p[1])*std::exp(-8.*viscosity*pi*pi*t);
...
public:
    ...
    penalty_factor = 1.0 * (dof_handler_u.get_fe().degree + 1) * (dof_handler_u.get_fe().degree);
  {
    ...
    dealii::FiniteElement<dim> const & fe      = dof_handler_u.get_fe();
    auto const reference_cells = dof_handler_u.get_fe().reference_cell();

    auto const quadrature = reference_cells.template get_gauss_type_quadrature<dim>(fe_degree_u + 1);
    dealii::FEValues<dim> fe_values(mapping, fe, quadrature, dealii::update_JxW_values);

    auto const face_quadrature =
      reference_cells.face_reference_cell(0).template get_gauss_type_quadrature<dim - 1>(fe_degree_u +
                                                                                            1);
  ...
  vmult(VectorType &dst, const VectorType &src) const
  {
    this->matrix_free.loop(
        &MomentumOperator::do_cell_integral_range,
  ...
  virtual void
  rhs(VectorType &dst, const VectorType &velocity_derivative, const VectorType &speed, const VectorType &pressure) const
  {
    this->matrix_free.loop(
        &MomentumOperator::do_rhs_cell_integral_range,
        &MomentumOperator::do_rhs_face_integral_range,
        &MomentumOperator::do_rhs_boundary_range,
        this,
        dst,
        std::vector<const VectorType *>{&velocity_derivative, &speed,
            &pressure},
        true,
        MatrixFree<dim, number>::DataAccessOnFaces::gradients,
        MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  ...
  void
    evaluate_vorticity(VectorType &dst, const VectorType &src) const
    {
      ...
      for (unsigned int cell = 0; cell < matrix_free.n_cell_batches(); ++cell)
        {
          eval_u.reinit(cell);
          eval_u.read_dof_values(dst); //TODO: check if we have to copy the vector here first
    ...
    for (unsigned int cell = range.first; cell < range.second; ++cell)
      {
        ...
        for (unsigned int q = 0; q < integrator.n_q_points; ++q)
         {
           const auto time_deriv = make_vectorized_array<number>(gamma0 / time_step)*integrator.get_value(q);
           ...
           integrator.submit_gradient(make_vectorized_array<number>(viscosity) * grad_u, q);
         }
        ...
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          { 
            const Tensor<1, dim, VectorizedArray<number>> solution_jump =
             ...
             const auto convective_flux = speed_normal * solution_average + make_vectorized_array<number>(0.5) * std::abs(speed_normal) * solution_jump;
            ...
            integrator_inner.submit_normal_derivative(-solution_jump *
                make_vectorized_array<number>(viscosity) * number(0.5),
                                                      q);
          ...
          for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
            {
              ...
              const auto convective_flux = (std::abs(speed) - speed) * integrator_inner.get_value(q);
    ...
    for (unsigned int face = range.first; face < range.second; ++face)
      {
        ...
        integrator_inner_p.gather_evaluate(*src[2],
                                         EvaluationFlags::values);

        integrator_speed_outer.gather_evaluate(*src[1], EvaluationFlags::values);
        integrator_outer_p.gather_evaluate(*src[2],
                                         EvaluationFlags::values);
        ...
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          { 
            ...
            speeds_faces(face, q) = 
            make_vectorized_array<number>(0.5)* (integrator_speed_inner.get_value(q) + integrator_speed_outer.get_value(q)) * normal;


            //const Tensor<1, dim, VectorizedArray<number>> p_avg =
            //number(0.5)*(integrator_inner_p.get_value(q) + integrator_outer_p.get_value(q)) * integrator_inner.normal_vector(q);
    ...
    for (unsigned int face = range.first; face < range.second; ++face)
      {
        ...
        if (matrix_free.get_boundary_id(face) == 0)
        {
          for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          {
            ...
            {
              const auto u_plus_m =
              evaluate_function(exact_velocity_m, integrator_inner.quadrature_point(q));
              const auto u_plus_m2 =
              evaluate_function(exact_velocity_m2, integrator_inner.quadrature_point(q));
              auto extrapolated_velocity = 2.0 * u_plus_m - u_plus_m2;
              speed_normal =
              0.5 * (normal * (integrator_speed_inner.get_value(q) + extrapolated_velocity));
            }
            else
            {
              speed_normal = make_vectorized_array<number>(0.5)* (integrator_speed_inner.get_value(q) + g) * normal;
            }
        ...
        else if (matrix_free.get_boundary_id(face) == 1)
        {
          for (const unsigned int q : integrator_inner.quadrature_point_indices())
          {
            ...
            const auto grad_g =
              evaluate_tensor_function(exact_velocity, integrator_inner.quadrature_point(q));
            const auto h_u = make_vectorized_array(viscosity) * grad_g * normal;

            const auto p_plus =
              evaluate_scalar_function(exact_pressure, integrator_inner.quadrature_point(q));
...
public:
  ...
  reinit(const MatrixFree<dim, number> &matrix_free, const unsigned int bdf_order_in)
  {
    ...
    const double penalty_factor = 1.0 * (fe_degree + 1) * (fe_degree);
    {
      ...
      dealii::FiniteElement<dim> const & fe      = matrix_free.get_dof_handler(dof_no_p).get_fe();
      auto const reference_cells = matrix_free.get_dof_handler(dof_no_p).get_fe().reference_cell();
      MappingQ1<dim> mapping;

      auto const quadrature = reference_cells.template get_gauss_type_quadrature<dim>(fe_degree + 1);
      dealii::FEValues<dim> fe_values(mapping, fe, quadrature, dealii::update_JxW_values);

      auto const face_quadrature =
        reference_cells.face_reference_cell(0).template get_gauss_type_quadrature<dim - 1>(fe_degree +
                                                                                              1);
      dealii::FEFaceValues<dim> fe_face_values(mapping, fe, face_quadrature, dealii::update_JxW_values);
  ...
  void
  compute_rhs(VectorType &dst, const VectorType &velocity, const VectorType &vorticity, const VectorType &u_extrapolated)
  {
    matrix_free->loop(&PressureOperator::local_rhs_domain,
                      ...
                      this,
                      dst,
                      std::vector<const VectorType *>{&velocity, &vorticity, &u_extrapolated},
                      true,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients,
                      MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }    
...
private:
    ...
    for (unsigned int face = face_range.first; face < face_range.second; face++)
    {
      ...
      else if (data.get_boundary_id(face) == 1)
      {
        for (const unsigned int q : eval_minus.quadrature_point_indices())
        {
          ...
          const auto viscous_value_flux = 2.0 * penalty_factor * u_minus -
                                          eval_minus.get_normal_derivative(q);
                   ...
                   const std::pair<unsigned int, unsigned int> & cell_range) const
  {
      ...
      for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
          ...
          eval_u_extrap.gather_evaluate(*src[2], EvaluationFlags::values);
          ...
          for (const unsigned int q : eval_p.quadrature_point_indices())
          {
              ...
              const auto convective_flux = 
                use_extrapolated_velocity ? 
                  eval_u.get_gradient(q) * eval_u_extrap.get_value(q):
                  eval_u.get_gradient(q) * eval_u.get_value(q);
                       ...
                       const std::pair<unsigned int, unsigned int> &face_range) const
  {
      ...
      for (unsigned int face = face_range.first; face < face_range.second; face++)
      {
          ...
          eval_u_extrap_minus.gather_evaluate(*src[2], EvaluationFlags::values);
          eval_u_extrap_plus.gather_evaluate(*src[2], EvaluationFlags::values);
          ...
          for (const unsigned int q : eval_p_minus.quadrature_point_indices())
            {
              ...
              { 
                ...
                const auto grad_flux = 0.5 * (eval_u_minus.get_value(q) - eval_u_plus.get_value(q)); 
      ...
      for (unsigned int face = face_range.first; face < face_range.second; face++)
      {
        if (data.get_boundary_id(face) == 0)
        {
          ...
          { 
            ...
            if (false)
            {
              curl_omega = make_vectorized_array<number>(4.0 * numbers::PI * numbers::PI) * g;
            }
            ...
            if(use_extrapolated_velocity)
            {
              ...
              const auto value_flux = -u_plus - u_minus_grad * (u_minus - g) 
              - viscosity * curl_omega;
...
do_test(const unsigned int fe_degree, const unsigned int n_refinements)
{
    ...
    LinearAlgebra::distributed::Vector<Number> vec_u, vec_u_n0, vec_u_n1, vec_u_deriv, vec_u_rhs, vec_p, vec_p_n0, vec_p_n1, vec_p_extrapolated, vec_u_norm, vec_rhs_norm, speed, vec_vorticity, vec_p_rhs, vec_p_norm;
    momentum_op.initialize_dof_vector(vec_u, dof_no_v);
    momentum_op.initialize_dof_vector(vec_u_n0, dof_no_v);
    momentum_op.initialize_dof_vector(vec_u_n1, dof_no_v);
    ...
    momentum_op.initialize_dof_vector(vec_p_n0, dof_no_p);
    momentum_op.initialize_dof_vector(vec_p_n1, dof_no_p);
    momentum_op.initialize_dof_vector(vec_p_extrapolated, dof_no_p);
    momentum_op.initialize_dof_vector(vec_u_norm, dof_no_v);
    momentum_op.initialize_dof_vector(vec_rhs_norm, dof_no_v);
    momentum_op.initialize_dof_vector(speed, dof_no_v);
    ...
    VectorTools::interpolate(mapping, dof_handler_u, exact_velocity, vec_u_n1);
    exact_velocity.set_time(time_step);
    VectorTools::interpolate(mapping, dof_handler_u, exact_velocity, vec_u_n0);

    AnalyticalSolutionPressure<dim> exact_pressure(u_x_max, viscosity);
    exact_pressure.set_time(0);
    VectorTools::interpolate(mapping, dof_handler_p, exact_pressure, vec_p_n1);
    exact_pressure.set_time(time_step);
    VectorTools::interpolate(mapping, dof_handler_p, exact_pressure, vec_p_n0);

    Number current_time = time_step;
    ...
    while(current_time <= end_time)
    {
        ...
        vec_p_extrapolated.add(-1.0, vec_p_n1);
        vec_p_extrapolated.add(2.0, vec_p_n0);
        

        speed = 0.;
        //exact_velocity.set_time(current_time);
        //VectorTools::interpolate(mapping, dof_handler_u, exact_velocity, speed);
        speed.add(-1.0, vec_u_n1);
        speed.add(2.0, vec_u_n0);

        vec_u_deriv = 0.;
        vec_u_deriv.add(-0.5 / time_step, vec_u_n1);
        vec_u_deriv.add(2.0 / time_step, vec_u_n0);
        
        vec_u_rhs = 0.;
        momentum_op.rhs(vec_u_rhs, vec_u_deriv, speed, vec_p_extrapolated);
        ...
        if (write_output)
        pcout<< "Pressure solver: " << control.last_step() << " iterations"
                  << std::endl;
        ...
        vec_u_n1.swap(vec_u_n0);
        vec_u_n0.swap(vec_u);

        vec_p_n1.swap(vec_p_n0);
        vec_p_n0.swap(vec_p);
        ...
        if (write_output)
        {
          ...
          VectorTools::compute_global_error(tria, error_per_cell, VectorTools::L2_norm);
            ...
            pcout << "L2 error velocity/pressure: " << velocity_error/velocity_norm << " " << pressure_error/pressure_norm << std::endl;
            pcout << std::endl;
        }
    }
    ...
    const double velocity_error =
    VectorTools::compute_global_error(tria, error_per_cell, VectorTools::L2_norm); //H1_seminorm);
  ...
  VectorTools::compute_global_error(tria, error_per_cell, VectorTools::L2_norm);
    ...
    VectorTools::integrate_difference(mapping,
        ...
        VectorTools::L2_norm); //H1_seminorm);
    const double velocity_norm =
    VectorTools::compute_global_error(tria, norm_per_cell, VectorTools::L2_norm); //H1_seminorm);
    ...
    exact_velocity.set_time(current_time);
    VectorTools::interpolate(mapping, dof_handler_u, exact_velocity, vec_u);
    vec_u_n0.add(-1.0, vec_u);

    exact_pressure.set_time(current_time);
    VectorTools::interpolate(mapping, dof_handler_p, exact_pressure, vec_p);
    vec_p_n0.add(-1.0, vec_p);


    pcout << "L2 error velocity/pressure: " << velocity_error/velocity_norm << " " << pressure_error/pressure_norm << std::endl;
    pcout << "realative error vectornorm: " << vec_u_n0.l2_norm()/vec_u.l2_norm() << " " << vec_p_n0.l2_norm()/vec_p.l2_norm() << std::endl;
    pcout << std::endl;
}
