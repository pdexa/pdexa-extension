...
public:
  ...
  void
  set_body_force_factory(
    std::function<std::unique_ptr<Function<dim>>()> &&body_force_in)
  {
      ...
      const VectorType &pressure) const
  {
    this->matrix_free.loop(
      ...
      this,
      dst,
      std::vector<const VectorType *>{&velocity_derivative, &speed, &pressure},
      true,
      MatrixFree<dim, number>::DataAccessOnFaces::gradients,
      MatrixFree<dim, number>::DataAccessOnFaces::gradients);
  }
  ...
  {
    this->matrix_free.loop(
      &MomentumOperator::local_divergence_domain,
  ...
  get_system_matrix(TrilinosWrappers::SparseMatrix &system_matrix)
  {
    ...
    if (is_dg)
      MatrixFreeTools::
        compute_matrix<dim, -1, 0, dim, number, VectorizedArray<number>>(
          matrix_free,
          AffineConstraints<number>(),
          system_matrix,
                   ...
                   Number     &max_vorticity)
  {
    ...
    this->matrix_free.cell_loop(
      &MomentumOperator::do_energy_cell_integral_range, this, dst, velocity);
  ...
  cacheFunctions()
  {
    ...
    if (dirichletBC_velocity_factory)
      ...
      AssertThrow(
        false, dealii::ExcMessage("Velocity BC factory empty at call site!"));
    if (dirichletBC_pressure_factory)
      ...
      AssertThrow(
        false, dealii::ExcMessage("Pressure BC factory empty at call site!"));
  }
...
private:
    ...
    for (const unsigned int q : integrator.quadrature_point_indices())
      {
        ...
        const auto time_deriv =
          make_vectorized_array<number>(gamma0 / time_step) * u;
        ...
        if (use_skew_symmetric_convective_formulation)
          {
            ...
            const auto convective_gradient_flux =
              -0.5 * outer_product(u, speed);

            integrator.submit_value(time_deriv + convective_value_flux, q);
            integrator.submit_gradient(
              div_penalty + make_vectorized_array<number>(viscosity) * grad_u +
    ...
    for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
      {
        ...
        const auto speed_outer =
          speeds_outer_faces(integrator_outer.get_cell_or_face_batch_id(), q);
        const auto speed_normal    = speed * normal;
        const auto convective_flux = speed_normal * solution_average +
                                     make_vectorized_array<number>(0.5) *
                                       std::abs(speed_normal) * solution_jump;
        ...
        if (use_skew_symmetric_convective_formulation)
          {
            ...
            integrator_inner.submit_value(continuity_penalty_value +
                                          ...
                                          q);
            integrator_outer.submit_value(-continuity_penalty_value -
                                            test_by_value +
        ...
        else if (use_divergence_formulation)
          {
            ...
            integrator_inner.submit_value(continuity_penalty_value +
                                            test_by_value + convective_div_flux,
                                          q);
            integrator_outer.submit_value(-continuity_penalty_value -
                                            test_by_value - convective_div_flux,
                                          q);
        ...
        integrator_inner.submit_normal_derivative(
          -solution_jump * make_vectorized_array<number>(viscosity) *
            number(0.5),
          q);
    ...
    FEFaceEvaluation<dim, -1, 0, n_components, Number> &integrator_inner) const
  {
    ...
    // Dirichlet boundary
    if (integrator_inner.boundary_id() == 0 ||
        integrator_inner.boundary_id() == 2)
      {
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          {
            const auto normal = integrator_inner.normal_vector(q);
            ...
            const auto convective_flux =
              (std::abs(speed_normal) - speed_normal) *
              integrator_inner.get_value(q);
            ...
            const auto continuity_penalty_value =
              2. * cont_pen * ((integrator_inner.get_value(q) - g) * normal) *
              normal;
            ...
            if (use_skew_symmetric_convective_formulation)
              {
                const auto speed =
                  speeds_faces(integrator_inner.get_cell_or_face_batch_id(), q);
                const auto convective_flux =
                  speed * (integrator_inner.get_value(q) *
            ...
            else if (use_divergence_formulation)
              {
                ...
                const auto boundary_flux =
                  (speed * integrator_inner.normal_vector(q)) *
                  integrator_inner.get_value(q);
    ...
    for (unsigned int cell = range.first; cell < range.second; ++cell)
      {
        ...
        penalty_factor_divergence[cell] =
          penalty_divergence * avg_velocity *
          std::exp(std::log(volume) / (number)dim) / ((number)fe_degree_u + 1);
        ...
        for (unsigned int q = 0; q < integrator.n_q_points; ++q)
          {
            ...
            const auto f =
              evaluate_function((*rhs), integrator.quadrature_point(q));
    ...
    for (unsigned int face = range.first; face < range.second; ++face)
      {
        ...
        integrator_speed_inner.gather_evaluate(*src[1],
                                               EvaluationFlags::values);
        integrator_inner_p.gather_evaluate(*src[2], EvaluationFlags::values);

        integrator_speed_outer.gather_evaluate(*src[1],
                                               EvaluationFlags::values);
        ...
        for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
          {
            ...
            // integrator_outer_p.get_value(q)) *
            // integrator_inner.normal_vector(q);
            const Tensor<1, dim, VectorizedArray<number>> p_jump =
              number(0.5) *
              (integrator_inner_p.get_value(q) -
               integrator_outer_p.get_value(q)) *
              normal;
        ...
        if (matrix_free.get_boundary_id(face) == 0 ||
            ...
            for (unsigned int q = 0; q < integrator_inner.n_q_points; ++q)
              {
                ...
                const auto convective_flux =
                  (std::abs(speed_normal) - speed_normal) * g;

                const auto value_flux =
                  make_vectorized_array<number>(2.0 * viscosity) * sigma * g;
                const auto gradient_flux =
                  make_vectorized_array<number>(viscosity) * g;
                ...
                if (use_skew_symmetric_convective_formulation)
                  {
                    ...
                    integrator_inner.submit_value(value_flux - p +
                                                    0.5 * convective_flux +
                                                    0.5 * convective_value_flux,
                                                  q);
                  }
        ...
        else if (matrix_free.get_boundary_id(face) == 1)
          {
            for (const unsigned int q :
                 integrator_inner.quadrature_point_indices())
              {
                speeds_faces(face, q) = integrator_speed_inner.get_value(q);
                const auto normal     = integrator_inner.normal_vector(q);

                const auto grad_g = evaluate_tensor_function(
                  (*velocity_bc), integrator_inner.quadrature_point(q));
                const auto h_u =
                  make_vectorized_array(viscosity) * grad_g * normal;

                const auto p_plus = evaluate_scalar_function(
                  (*pressure_bc), integrator_inner.quadrature_point(q));
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        for (unsigned int q = 0; q < eval_p_minus.n_q_points; ++q)
          {
            const auto jump =
              -0.5 * (eval_p_minus.get_value(q) - eval_p_plus.get_value(q)) *
              eval_p_minus.normal_vector(q);
        ...
        if (data.get_boundary_id(face) == 0 || data.get_boundary_id(face) == 2)
        ...
        else
          {
            ...
            for (const unsigned int q : eval_u_minus.quadrature_point_indices())
              {
                const auto value_flux =
                  -eval_p_minus.get_value(q) * eval_p_minus.normal_vector(q);
        ...
        for (const unsigned int q : eval_p_minus.quadrature_point_indices())
          {
            ...
            const auto div_factor =
              0.5 * (eval_u_minus.get_value(q) + eval_u_plus.get_value(q)) *
              normal;
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        VectorizedArray<Number> volume_vec =
          dealii::make_vectorized_array<Number>(0.);
        VectorizedArray<Number> energy_vec =
          dealii::make_vectorized_array<Number>(0.);
        VectorizedArray<Number> enstrophy_vec =
          dealii::make_vectorized_array<Number>(0.);
        ...
        for (unsigned int q = 0; q < fe_eval.n_q_points; ++q)
          {
            ...
            dealii::Tensor<1, dim == 3 ? dim : 1, VectorizedArray<Number>>
              omega = fe_eval.get_curl(q);
...
public:
         ...
         const bool use_traction_boundary_condition_in = false)
  {
    ...
    is_dg =
      matrix_free->get_dof_handler(dof_no_p).get_fe().n_dofs_per_vertex() == 0;
                    ...
                    TrilinosWrappers::SparseMatrix  &system_matrix)
  {
    ...
    if (is_dg)
      MatrixFreeTools::
        compute_matrix<dim, -1, 0, 1, number, VectorizedArray<number>>(
          *matrix_free,
          constraints,
          system_matrix,
...
private:
  ...
  void
  local_apply_domain_matrix_based(
    FEEvaluation<dim, -1, 0, 1, number> &eval) const
  {
            ...
            for (const unsigned int q : eval_p_minus.quadrature_point_indices())
              {
                ...
                const auto g =
                  evaluate_function((*velocity_bc),
                                    eval_p_minus.quadrature_point(q));
                auto u_plus =
                  make_vectorized_array(integration_constants.get_gamma0() /
                                        time_step) *
                  g;

                if (!use_leray_projection)
                  for (unsigned int i = 0;
                       i < integration_constants.get_order();
                       ++i)
                    {
                      velocity_bc->set_time(time - (i + 1) * time_step);
                      u_plus -=
                        make_vectorized_array(
                          integration_constants.get_alpha(i) / time_step) *
                        evaluate_function((*velocity_bc),
                                          eval_p_minus.quadrature_point(q));
                    }
                ...
                if constexpr (dim == 3)
                  ...
                  {
                    // curl is a scalar in 2d, but evaluator still returns a
                    // vector where only the first component is non-zero
                    curl_omega[0] =
                ...
                if (use_leray_projection)
                ...
                else
                  {
                    const auto g =
                      evaluate_function((*velocity_bc),
                                        eval_p_minus.quadrature_point(q));
                    eval_p_minus.submit_value(g * eval_p_minus.normal_vector(q),
                                              q);
                  }
              }
