...
constexpr custom_operator_item extract_batch_item(custom_operator_view op, gko::size_type batch_id) {
...
// TODO: make this either a class member or use CPO
[[nodiscard]] dummy::custom_operator_view create_view(const dummy::CustomOperator* x) {
  return {x->get_num_batch_items(),
          static_cast<gko::int32>(x->get_common_size()[0]),
          static_cast<gko::int32>(x->get_common_size()[1])};
}
...
constexpr void advanced_apply(double alpha,
  ...
  for (gko::size_type row = 0; row < static_cast<gko::size_type>(a.num_rows); ++row) {
    double acc{};

    if (row > 0) {
      acc += -gko::one<double>() * b.values[row - 1];
    }
    acc += (2.0 + static_cast<double>(a.batch_id) / static_cast<double>(num_batches)) * b.values[row];
    if (row < static_cast<gko::size_type>(a.num_rows - 1)) {
      acc += -gko::one<double>() * b.values[row + 1];
    }
...
__device__ void advanced_apply(double alpha,
  ...
  for (gko::size_type row = tidx; row < a.num_rows; row += blockDim.x) {
    ...
    acc +=
      (static_cast<double>(2.0) + static_cast<double>(a.batch_id) / static_cast<double>(num_batches)) * b.values[row];
    if (row < a.num_rows - 1) {
      acc += -gko::one<double>() * b.values[row + 1];
    }
...
int main(int argc, char* argv[]) {
  ...
  options.add_options()(
    "executor", "The Ginkgo Executor type", cxxopts::value<std::string>()->default_value("reference"))(
...
#pragma omp parallel for
  ...
  using SolverVariant = std::variant<std::shared_ptr<cg>, std::shared_ptr<cg_op>>;
