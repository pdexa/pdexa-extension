...
constexpr void advanced_apply(
  ...
  for (gko::size_type row = 0; row < static_cast<gko::size_type>(a.num_rows); ++row) {
    ...
    if (row > 0) { acc += -gko::one<double>() * b.values[row - 1]; }
    acc += (2.0 + static_cast<double>(a.batch_id) / static_cast<double>(num_batches)) *
      b.values[row];
    if (row < static_cast<gko::size_type>(a.num_rows - 1)) { acc += -gko::one<double>() * b.values[row + 1]; }
