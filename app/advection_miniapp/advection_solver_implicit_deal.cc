...
template<typename T>
using VectorizedArray = VectorizedArray<T>;
...
public:
  CellwiseOperator(const Tensor<2, dim, VectorizedArray<Number>>& jac,
      ...
      jac(jac), shape(shape), speed_cells(speed_cells), normal_speed_faces(normal_speed_faces),
      cell_quadrature(cell_quadrature), face_quadrature(face_quadrature), inv_dt(inv_dt), time_factor(time_factor) {}

  void vmult(Vector<Number>& dst, const Vector<Number>& src) const {
    const VectorizedArray<Number>* src_ptr = reinterpret_cast<const VectorizedArray<Number>*>(src.data());
    VectorizedArray<Number>* dst_ptr = reinterpret_cast<VectorizedArray<Number>*>(dst.data());
    const unsigned int dofs_per_component = Utilities::pow(fe_degree + 1, dim);
    ...
    for (unsigned int d = 0; d < dim; ++d) {
      ...
      const unsigned int offset1 = (dim > 2 ? (d == 2 ? (fe_degree + 1) : Utilities::pow(fe_degree + 1, 2)) : 1);
      ...
      for (unsigned int i1 = 0; i1 < (dim > 2 ? fe_degree + 1 : 1); ++i1)
        for (unsigned int i0 = 0; i0 < (dim > 1 ? fe_degree + 1 : 1); ++i0) {
          ...
          for (unsigned int j = 0; j < fe_degree + 1; ++j, my_ptr += stride) {
            const VectorizedArray<Number> value = my_ptr[0];
            sum_l += shape.quadrature_data_on_face[0][j] * value;
            sum_r += shape.quadrature_data_on_face[0][fe_degree - j] * value;
          }

          const unsigned int q = (dim == 3 && d == 1) ? i0 * (fe_degree + 1) + i1 : i1 * (fe_degree + 1) + i0;
          {
            ...
            sum_l = sum_l * coefficient * (face_quadrature.weight(q) * surface_JxW);
          }
          {
            const auto speed = normal_speed_faces(2 * d + 1, q) * time_factor;
            const auto coefficient = 0.5 * (speed + flux_alpha * std::abs(speed)) - factor_skew * speed;
            sum_r = sum_r * coefficient * (face_quadrature.weight(q) * surface_JxW);
          }

          auto out_ptr = dst_ptr + offset1 * i1 + offset0 * i0;
          if (d == 0)
            for (unsigned int j = 0; j < fe_degree + 1; ++j, out_ptr += stride)
              out_ptr[0] =
                shape.quadrature_data_on_face[0][j] * sum_l + shape.quadrature_data_on_face[0][fe_degree - j] * sum_r;
          else
            for (unsigned int j = 0; j < fe_degree + 1; ++j, out_ptr += stride)
              out_ptr[0] +=
                shape.quadrature_data_on_face[0][j] * sum_l + shape.quadrature_data_on_face[0][fe_degree - j] * sum_r;
    ...
    internal::EvaluatorTensorProduct<internal::evaluate_general, dim, fe_degree + 1, fe_degree + 1,
                                     VectorizedArray<Number>, Number>
      eval(shape.shape_values, shape.shape_gradients_collocation, AlignedVector<Number>());

    // volume integrals
    eval.template gradients<0, true, false>(src_ptr, gradients[0]);
    if (dim > 1) eval.template gradients<1, true, false>(src_ptr, gradients[1]);
    if (dim > 2) eval.template gradients<2, true, false>(src_ptr, gradients[2]);
    ...
    for (unsigned int q = 0; q < dofs_per_component; ++q) {
      ...
      VectorizedArray<Number> speed_gradu = speed[0] * gradients[0][q] * jac[0][0];
      for (unsigned int d = 1; d < dim; ++d) speed_gradu += speed[d] * gradients[d][q] * jac[d][d];
      VectorizedArray<Number> flux = (factor_skew * time_factor) * speed_gradu;
      const VectorizedArray<Number> result = (-1.0 + factor_skew) * time_factor * u * (JxW * cell_quadrature.weight(q));
      for (unsigned int d = 0; d < dim; ++d) gradients[d][q] = result * speed[d] * jac[d][d];
    ...
    if (dim > 1) eval.template gradients<1, false, true>(gradients[1], dst_ptr);
    if (dim > 2) eval.template gradients<2, false, true>(gradients[2], dst_ptr);
  }
...
public:
  ...
  CellwisePreconditionerFDM(const std::array<FullMatrix<std::complex<double>>, 2>& eigenvectors,
    ...
    for (unsigned int i2 = 0, c = 0; i2 < (dim > 2 ? n : 1); ++i2)
      for (unsigned int i1 = 0; i1 < (dim > 1 ? n : 1); ++i1)
        for (unsigned int i0 = 0; i0 < n; ++i0, ++c) {
          ...
          std::complex<VectorizedArray<Number>> diagonal_element = VectorizedArray<Number>();
          for (unsigned int d = 0; d < dim; ++d) {
            std::complex<VectorizedArray<Number>> eig1(eigenvalues[0][indices[d]]), eig2(eigenvalues[1][indices[d]]);
            diagonal_element += average_velocity[d] * ((1.0 - blend_factor_eig[d]) * eig1 + blend_factor_eig[d] * eig2);
          }
          inverse_eigenvalues_for_cell[c] = inv_jacobian_determinant / (VectorizedArray<Number>(inv_dt) + diagonal_element);
        }
    for (unsigned int d = 0; d < dim; ++d) {
      for (unsigned int i = 0, c = 0; i < n; ++i)
        for (unsigned int j = 0; j < n; ++j, ++c) {
          {
            const std::complex<VectorizedArray<Number>> eig1(eigenvectors[0](i, j)), eig2(eigenvectors[1](i, j));
            this->eigenvectors[d][c] = (1.0 - blend_factor_eig[d]) * eig1 + blend_factor_eig[d] * eig2;
          }
          {
            const std::complex<VectorizedArray<Number>> eig1(inverse_eigenvectors[0](i, j)),
              eig2(inverse_eigenvectors[1](i, j));
            this->inverse_eigenvectors[d][c] = (1.0 - blend_factor_eig[d]) * eig1 + blend_factor_eig[d] * eig2;
          }
        }
    }
    /*
  ...
  void vmult(Vector<Number>& dst, const Vector<Number>& src) const {
    ...
    AssertDimension(VectorizedArray<Number>::size() * data_array.size(), dst.size());
    AssertDimension(VectorizedArray<Number>::size() * data_array.size(), src.size());
    ...
    using Eval = internal::EvaluatorTensorProduct<internal::evaluate_general, dim, n, n, vcomplex>;
    // apply V M^{-1}
    Eval::template apply<0, false, false>(inverse_eigenvectors[0].data(), data_array.data(), data_array.data());
    if (dim > 1)
      Eval::template apply<1, false, false>(inverse_eigenvectors[1].data(), data_array.data(), data_array.data());
    if (dim > 2)
      Eval::template apply<2, false, false>(inverse_eigenvectors[2].data(), data_array.data(), data_array.data());

    // apply inv(I x Lambda + Lambda x I)
    for (unsigned int c = 0; c < data_array.size(); ++c) data_array[c] *= inverse_eigenvalues_for_cell[c];

    // apply V^{-1}
    Eval::template apply<0, false, false>(eigenvectors[0].data(), data_array.data(), data_array.data());
    if (dim > 1) Eval::template apply<1, false, false>(eigenvectors[1].data(), data_array.data(), data_array.data());
    if (dim > 2) Eval::template apply<2, false, false>(eigenvectors[2].data(), data_array.data(), data_array.data());

    // copy back to real vector
    for (unsigned int i = 0; i < data_array.size(); ++i)
      data_array[i].real().store(dst.begin() + i * VectorizedArray<Number>::size());
  }
...
void transform_to_collocation(const internal::MatrixFreeFunctions::UnivariateShapeData<Number>& shape,
  ...
  internal::EvaluatorTensorProduct<internal::evaluate_general, dim, fe_degree + 1, fe_degree + 1,
                                   VectorizedArray<Number>, Number>
    evaluator(AlignedVector<Number>(), AlignedVector<Number>(), shape.inverse_shape_values);
  VectorizedArray<Number>* dst_ptr = reinterpret_cast<VectorizedArray<Number>*>(dst.data());
  ...
  if (dim > 1) evaluator.template hessians<1, true, false>(out, out);
  if (dim > 2) evaluator.template hessians<2, true, false>(out, out);
}
...
void transform_from_collocation(const internal::MatrixFreeFunctions::UnivariateShapeData<Number>& shape,
  ...
  internal::EvaluatorTensorProduct<internal::evaluate_evenodd, dim, fe_degree + 1, fe_degree + 1,
                                   VectorizedArray<Number>, Number>
    evaluator(AlignedVector<Number>(), AlignedVector<Number>(), shape.inverse_shape_values_eo);
  const VectorizedArray<Number>* src_ptr = reinterpret_cast<const VectorizedArray<Number>*>(src.data());
  ...
  if (dim > 1) evaluator.template hessians<1, false, false>(out, out);
  if (dim > 2) evaluator.template hessians<2, false, false>(out, out);
}
...
public:
  ...
  const MatrixFree<dim, Number, VectorizedArray<Number>>& get_matrix_free() const { return data; }
  ...
  void compute_rhs(LinearAlgebra::distributed::Vector<Number>& dst,
    ...
    data.loop(&AdvectionOperation<dim, fe_degree>::local_rhs_domain,
              ...
              &AdvectionOperation<dim, fe_degree>::local_rhs_boundary_face, this, dst, src, true,
              MatrixFree<dim, Number, VectorizedArray<Number>>::DataAccessOnFaces::values,
              MatrixFree<dim, Number, VectorizedArray<Number>>::DataAccessOnFaces::values);
    dst *= -1. / time_step;
  ...
  void vmult(LinearAlgebra::distributed::Vector<Number>& dst,
    ...
    data.loop(&AdvectionOperation<dim, fe_degree>::local_apply_domain,
              ...
              &AdvectionOperation<dim, fe_degree>::local_apply_boundary_face, this, dst, src, true,
              MatrixFree<dim, Number, VectorizedArray<Number>>::DataAccessOnFaces::values,
              MatrixFree<dim, Number, VectorizedArray<Number>>::DataAccessOnFaces::values);
...
private:
  MatrixFree<dim, Number, VectorizedArray<Number>> data;
  double time;
  double time_step;
  ...
  void local_apply_domain(const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
                          LinearAlgebra::distributed::Vector<Number>& dst,
                          const LinearAlgebra::distributed::Vector<Number>& src,
                          const std::pair<unsigned int, unsigned int>& cell_range) const;

  void local_apply_inner_face(const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
                              LinearAlgebra::distributed::Vector<Number>& dst,
                              const LinearAlgebra::distributed::Vector<Number>& src,
                              const std::pair<unsigned int, unsigned int>& cell_range) const;
  void local_apply_boundary_face(const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
                                 LinearAlgebra::distributed::Vector<Number>& dst,
                                 const LinearAlgebra::distributed::Vector<Number>& src,
                                 const std::pair<unsigned int, unsigned int>& cell_range) const;

  void local_rhs_domain(const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
                        LinearAlgebra::distributed::Vector<Number>& dst,
                        const LinearAlgebra::distributed::Vector<Number>& src,
                        const std::pair<unsigned int, unsigned int>& cell_range) const;

  void local_rhs_inner_face(const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
                            LinearAlgebra::distributed::Vector<Number>& dst,
                            const LinearAlgebra::distributed::Vector<Number>& src,
                            const std::pair<unsigned int, unsigned int>& cell_range) const;
  void local_rhs_boundary_face(const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
                               LinearAlgebra::distributed::Vector<Number>& dst,
                               const LinearAlgebra::distributed::Vector<Number>& src,
                               const std::pair<unsigned int, unsigned int>& cell_range) const;
};
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::reinit(const DoFHandler<dim>& dof_handler) {
  ...
  {
    FEEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval(data);
  ...
  {
    FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval(data, true);
  ...
  for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell) {
    ...
    for (unsigned int q = 0; q < eval.n_q_points; ++q) {
      average_velocity += eval.inverse_jacobian(q) * speeds_cells(cell, q) * eval.JxW(q);
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::local_apply_domain(
  ...
  for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
    ...
    for (unsigned int q = 0; q < eval.n_q_points; ++q) {
      ...
      Tensor<1, dim, VectorizedArray<Number>> volume_flux = ((-1.0 + factor_skew) * speed) * (factor_time * u);
      eval.submit_gradient(volume_flux, q);
      VectorizedArray<Number> volume_val = ((factor_skew * speed) * gradu) * factor_time + inv_dt * u;
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::local_apply_inner_face(
  const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
  LinearAlgebra::distributed::Vector<Number>& dst,
  const LinearAlgebra::distributed::Vector<Number>& src,
  ...
  FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_minus(data, true);
  FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_plus(data, false);
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::local_apply_boundary_face(
  const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
  LinearAlgebra::distributed::Vector<Number>& dst,
  const LinearAlgebra::distributed::Vector<Number>& src,
  const std::pair<unsigned int, unsigned int>& face_range) const {
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::local_rhs_domain(
  const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
  LinearAlgebra::distributed::Vector<Number>& dst,
  const LinearAlgebra::distributed::Vector<Number>& src,
  const std::pair<unsigned int, unsigned int>& cell_range) const {
  FEEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval(data);
  FEEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_src(data);
  ...
  for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
    ...
    for (unsigned int q = 0; q < eval.n_q_points; ++q) {
      ...
      Tensor<1, dim, VectorizedArray<Number>> volume_flux = ((-1.0 + factor_skew) * speed * u) * factor_time;
      eval.submit_gradient(volume_flux, q);
      const VectorizedArray<Number> volume_val = (factor_skew * (speed * gradu)) * factor_time;
      eval.submit_value(volume_val, q);
    }
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::local_rhs_inner_face(
  const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
  LinearAlgebra::distributed::Vector<Number>& dst,
  const LinearAlgebra::distributed::Vector<Number>& src,
  const std::pair<unsigned int, unsigned int>& face_range) const {
  ...
  FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_minus(data, true);
  FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_plus(data, false);
  FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_src_minus(data, true);
  FEFaceEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> eval_src_plus(data, false);
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::local_rhs_boundary_face(
  const MatrixFree<dim, Number, VectorizedArray<Number>>& data,
  LinearAlgebra::distributed::Vector<Number>& dst,
  const LinearAlgebra::distributed::Vector<Number>& src,
  const std::pair<unsigned int, unsigned int>& face_range) const {
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::project_initial(LinearAlgebra::distributed::Vector<Number>& dst) const {
  ExactSolution<dim> solution(0.);
  FEEvaluation<dim, fe_degree, fe_degree + 1, 1, Number, VectorizedArray<Number>> phi(data);
  MatrixFreeOperators::CellwiseInverseMassMatrix<dim, fe_degree, 1, Number, VectorizedArray<Number>> inverse(phi);
...
template<int dim, int fe_degree>
Tensor<1, 3> AdvectionOperation<dim, fe_degree>::compute_mass_and_energy(
...
template<int dim, int fe_degree>
void AdvectionOperation<dim, fe_degree>::precondition_block_jacobi(
  ...
  Vector<double> local_src(eval.dofs_per_cell * VectorizedArray<Number>::size());
  Vector<double> local_dst(eval.dofs_per_cell * VectorizedArray<Number>::size());

  // CellwisePreconditioner<Number> precondition(
  //  data.get_mapping_info().cell_data[0].descriptor[0].quadrature);

  const unsigned int n_max_iterations = 20;
  typename SolverGMRES<Vector<Number>>::AdditionalData gmres_data;
  gmres_data.right_preconditioning = false;
  gmres_data.orthogonalization_strategy = LinearAlgebra::OrthogonalizationStrategy::classical_gram_schmidt;
  ...
  for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell) {
    ...
    transform_to_collocation<dim, fe_degree>(data.get_shape_info().data[0], eval.begin_dof_values(), local_src);

    CellwiseOperator<dim, fe_degree> local_operator(
      eval.inverse_jacobian(0), data.get_shape_info().data[0], &speeds_cells(cell, 0), normal_speeds_faces[cell],
      data.get_mapping_info().cell_data[0].descriptor[0].quadrature,
      data.get_mapping_info().face_data[0].descriptor[0].quadrature, 1. / time_step, factor_time);
    CellwisePreconditionerFDM<dim, fe_degree> precondition(eigenvectors, inverse_eigenvectors, eigenvalues,
                                                           determinant(eval.inverse_jacobian(0)),
                                                           scaled_cell_velocity[cell] * factor_time, 1. / time_step);
    IdentityMatrix precondition_identity(eval.dofs_per_cell * VectorizedArray<Number>::size());
    Timer gmres_timer;

    IterationNumberControl control(n_max_iterations, 1e-14 * local_src.l2_norm(), false, false);
    SolverGMRES<Vector<Number>> gmres(control, memory, gmres_data);

    gmres.solve(local_operator, local_dst, local_src, precondition);
    computing_times[4] += gmres_timer.wall_time();

    transform_from_collocation<dim, fe_degree>(data.get_shape_info().data[0], local_dst, eval.begin_dof_values());
...
template<int dim>
void AdvectionProblem<dim>::setup_dofs() {
#if DEAL_II_VERSION_GTE(9, 3, 0)
...
template<int dim>
void AdvectionProblem<dim>::output_results(const unsigned int output_number, const Tensor<1, 3> mass_energy) {
...
std::array<double, 5>
compute_least_squares_fit_neq(OperatorType const& op, std::vector<VectorType> const& vectors, VectorType const& rhs) {
...
void AdvectionProblem<dim>::run() {
  ...
  */
  ...
  BlockJacobi<AdvectionOperation<dim, fe_degree>> precondition(advection_operator);
