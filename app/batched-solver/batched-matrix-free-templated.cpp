...
constexpr custom_operator_item extract_batch_item(custom_operator_view op, gko::int64 batch_id) {
  return {op.num_batch_items, batch_id, op.num_rows, op.num_cols};
}

constexpr void advanced_apply(double alpha,
                              custom_operator_item a,
                              gko::batch::multi_vector::batch_item<const double> b,
                              double beta,
  ...
  for (gko::int32 row = 0; row < a.num_rows; ++row) {
    double acc{};
...
constexpr void simple_apply(const custom_operator_item& a,
...
__device__ void advanced_apply(double alpha,
                               custom_operator_item a,
                               gko::batch::multi_vector::batch_item<const double> b,
                               double beta,
                               gko::batch::multi_vector::batch_item<double> x,
...
__device__ void simple_apply(const custom_operator_item& a,
                             const gko::batch::multi_vector::batch_item<const double>& b,
                             const gko::batch::multi_vector::batch_item<double>& x,
...
using bicg = gko::batch_template::solver::Bicgstab<mtx_type>;
using bicg_op = gko::batch_template::solver::Bicgstab<op_type>;
...
auto unbatch(const InputType* batch_object) {
  auto unbatched_mats = std::vector<std::unique_ptr<typename InputType::unbatch_type>>{};
...
int main(int argc, char* argv[]) {
  ...
  cxxopts::Options options("batched-matrix-free", "Solve a batched problem with the external matrix format.");

  options.add_options()("executor", "The Ginkgo Executor type",
                        cxxopts::value<std::string>()->default_value("reference"))(
    "b,batches", "The number of batches", cxxopts::value<value_type>()->default_value("2"))(
    "s,size", "The (square) size of a batch item", cxxopts::value<size_type>()->default_value("32"))(
    "print-residuals", "Print the final residuals",
    cxxopts::value<bool>()->default_value("false")->implicit_value("true"))(
    "matrix-free", "Use external matrix format", cxxopts::value<bool>()->default_value("true")->implicit_value("true"))(
    "solver", "The solver to use (Cg, or Bicgstab)",
    cxxopts::value<std::string>()->default_value("cg"))("h,help", "Show this message");
  ...
  auto batch_mat_size = gko::batch_dim<2>(num_systems, gko::dim<2>(num_rows, num_rows));
  auto batch_vec_size = gko::batch_dim<2>(num_systems, gko::dim<2>(num_rows, 1));
  auto nnz = 3 * (num_rows - 2) + 4;

  auto A_mtx = gko::share(mtx_type::create(exec->get_master(), batch_mat_size, nnz));
  auto A_op = std::make_shared<op_type>(exec, batch_mat_size);

  if (args["print-residuals"].as<bool>() || !args["matrix-free"].as<bool>()) {
    gko::matrix_data<value_type, index_type> md(batch_mat_size.get_common_size());
    auto create_batch = [batch_mat_size, num_rows, nnz](gko::size_type id, auto& md) {
      md.nonzeros.reserve(nnz);
      for (index_type i = 0; i < static_cast<index_type>(num_rows); ++i) {
        if (i > 0) { md.nonzeros.emplace_back(i, i - 1, -1); }
        md.nonzeros.emplace_back(
          i, i, 2 + static_cast<value_type>(id) / static_cast<value_type>(batch_mat_size.get_num_batch_items()));
        if (i < static_cast<index_type>(num_rows) - 1) { md.nonzeros.emplace_back(i, i + 1, -1); }
      }
    };
#pragma omp parallel for firstprivate(md)
    for (gko::size_type id = 0; id < batch_mat_size.get_num_batch_items(); ++id) {
      md.nonzeros.clear();
      create_batch(id, md);
  ...
  using SolverVariant =
    std::variant<std::shared_ptr<cg>, std::shared_ptr<cg_op>, std::shared_ptr<bicg>, std::shared_ptr<bicg_op>>;
  auto finalize_solver = [&](auto params, auto op) {
    return params.with_max_iterations(500)
      .with_tolerance(reduction_factor)
      .with_tolerance_type(gko::batch::stop::tolerance_type::relative)
      .on(exec)
      ->generate(op);
  };
  auto solver{args["matrix-free"].as<bool>()
                ? (args["solver"].as<std::string>() == "cg" ? SolverVariant(finalize_solver(cg_op::build(), A_op))
                                                            : SolverVariant(finalize_solver(bicg_op::build(), A_op)))
                : (args["solver"].as<std::string>() == "cg" ? SolverVariant(finalize_solver(cg::build(), A_mtx))
                                                            : SolverVariant(finalize_solver(bicg::build(), A_mtx)))};

  // @sect3{Batch logger}
  // Create a logger to obtain the iteration counts and "implicit" residual
  //  norms for every system after the solve.
  std::shared_ptr<const gko::batch::log::BatchConvergence<value_type>> logger =
    gko::batch::log::BatchConvergence<value_type>::create();

  // @sect3{Generate and solve}
  // add the logger to the solver
  std::visit([logger](auto& solver_v) { solver_v->add_logger(logger); }, solver);
  ...
  if (args["print-residuals"].as<bool>()) {
    ...
    auto host_log_resid = gko::make_temporary_clone(exec->get_master(), &logger->get_residual_norm());
    auto host_log_iters = gko::make_temporary_clone(exec->get_master(), &logger->get_num_iterations());
    ...
    for (size_type i = 0; i < num_systems; ++i) {
      std::cout << " System no. " << i << ": residual norm = " << unb_res[i]->at(0, 0)
                << ", implicit residual norm = " << host_log_resid->get_const_data()[i]
                << ", iterations = " << host_log_iters->get_const_data()[i] << std::endl;
      const real_type relresnorm = unb_res[i]->at(0, 0) / unb_bnorm[i]->at(0, 0);
      if (!(relresnorm <= reduction_factor)) {
        std::cout << "System " << i << " converged only to " << relresnorm << " relative residual." << std::endl;
      }
    }
  }
