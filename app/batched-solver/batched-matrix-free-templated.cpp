...
constexpr custom_operator_item extract_batch_item(custom_operator_view op, gko::int64 batch_id, gko::any_kernel) {
...
using prec_type = gko::batch_preconditioner::Identity<value_type>;
using bicg = gko::batch_template::solver::Bicgstab<mtx_type, prec_type>;
using bicg_op = gko::batch_template::solver::Bicgstab<op_type, prec_type>;
...
int main(int argc, char* argv[]) {
  ...
  options.add_options()("executor", "The Ginkgo Executor type",
    ...
    cxxopts::value<bool>()->default_value("true")->implicit_value("true"))("h,help", "Show this message");
...
#pragma omp parallel for firstprivate(md)
  ...
  using SolverVariant = std::variant<std::shared_ptr<bicg>, std::shared_ptr<bicg_op>>;
  auto finalize_solver = [&](auto params, auto op) {
    return params.with_max_iterations(500)
      ...
      .on(exec)
      ->generate(std::move(op));
  };
  auto solver{args["matrix-free"].as<bool>()
                ? SolverVariant(finalize_solver(gko::batch_template::solver::bicgstab::build(), A_op))
                : SolverVariant(finalize_solver(gko::batch_template::solver::bicgstab::build(), A_mtx))};
